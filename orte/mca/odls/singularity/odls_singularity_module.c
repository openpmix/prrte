...
const char *singularity_exec_argc_env_var_name = "SY_EXEC_ARGS";

/*
 * Module functions (function pointers used in a struct)
 */
static int orte_odls_singularity_launch_local_procs(opal_buffer_t *data);
static int orte_odls_singularity_kill_local_procs(opal_pointer_array_t *procs);
static int orte_odls_singularity_signal_local_procs(const orte_process_name_t *proc, int32_t signal);
static int orte_odls_singularity_restart_proc(orte_proc_t *child);
 ...
 * Module
 */
orte_odls_base_module_t orte_odls_singularity_module = {
    .get_add_procs_data = orte_odls_base_default_get_add_procs_data,
    .launch_local_procs = orte_odls_singularity_launch_local_procs,
    .kill_local_procs = orte_odls_singularity_kill_local_procs,
    .signal_local_procs = orte_odls_singularity_signal_local_procs,
    .restart_proc = orte_odls_singularity_restart_proc};

/* deliver a signal to a specified pid. */
static int odls_singularity_kill_local(pid_t pid, int signum)
{
    pid_t pgrp;
    ...
    if (0 != kill(pid, signum))
    {
        if (ESRCH != errno)
        {
            OPAL_OUTPUT_VERBOSE((2, orte_odls_base_framework.framework_output,
                                 "%s odls:singularity:SENT KILL %d TO PID %d GOT ERRNO %d",
                                 ORTE_NAME_PRINT(ORTE_PROC_MY_NAME), signum, (int)pid, errno));
            return errno;
        }
    }
    OPAL_OUTPUT_VERBOSE((2, orte_odls_base_framework.framework_output,
                         "%s odls:singularity:SENT KILL %d TO PID %d SUCCESS",
                         ORTE_NAME_PRINT(ORTE_PROC_MY_NAME), signum, (int)pid));
    return 0;
}

int orte_odls_singularity_kill_local_procs(opal_pointer_array_t *procs)
{
    int rc;

    if (ORTE_SUCCESS != (rc = orte_odls_base_default_kill_local_procs(procs,
                                                                      odls_singularity_kill_local)))
    {
        ORTE_ERROR_LOG(rc);
        return rc;
    }
...
static void set_handler_singularity(int sig)
{
...
static int do_child(orte_odls_spawn_caddy_t *cd, int write_fd)
{
    ...
    char dir[MAXPATHLEN];
    char *exec_args = NULL;
    char *singularity_bin_path = NULL;
    char **cmd_args = NULL;
        ...
        if (ORTE_FLAG_TEST(cd->jdata, ORTE_JOB_FLAG_FORWARD_OUTPUT))
        {
            if (ORTE_SUCCESS != (i = orte_iof_base_setup_child(&cd->opts, &cd->env)))
            {
                ...
                send_error_show_help(write_fd, 1,
                                     "help-orte-odls-singularity.txt",
                                     "iof setup failed",
    ...
    /* Set signal handlers back to the singularity.  Do this close to
    ...
    set_handler_singularity(SIGTERM);
    set_handler_singularity(SIGINT);
    set_handler_singularity(SIGHUP);
    set_handler_singularity(SIGPIPE);
    set_handler_singularity(SIGCHLD);

    /* Unblock all signals, for many of the same reasons that we
       set the singularity handlers, above.  This is noticable on
    ...
    {
        if (strncmp(cd->env[i], singularity_exec_argc_env_var_name, strlen(singularity_exec_argc_env_var_name)) == 0)
        {
            exec_args = &cd->env[i][strlen(singularity_exec_argc_env_var_name) + 1]; // We do not want the "="
            break;
        }
    ...
    singularity_bin_path = opal_path_findv("singularity", X_OK, cd->env, NULL);

    // We need to prepend cd->cmd to cd->argv because in our case, cd->cmd is the
    // path to the container's image and the implicit command is "singularity"

    opal_argv_append_nosize(&cmd_args, singularity_bin_path);
    ...
    while (cd->argv[i])
    {
        opal_argv_append_nosize(&cmd_args, cd->argv[i]);
        i++;
    }
...
static int do_parent(orte_odls_spawn_caddy_t *cd, int read_fd)
{
    ...
    while (1)
    {
        ...
        if (msg.file_str_len > 0)
        {
            ...
            if (OPAL_SUCCESS != rc)
            {
                orte_show_help("help-orte-odls-singularity.txt", "syscall fail",
                               true,
                               orte_process_info.nodename, cd->app->app,
 ...
 */
static int odls_singularity_fork_local_proc(void *cdptr)
{
...
int orte_odls_singularity_launch_local_procs(opal_buffer_t *data)
{
    int rc;
    orte_jobid_t job;
    ...
    {
        OPAL_OUTPUT_VERBOSE((2, orte_odls_base_framework.framework_output,
                             "%s odls:singularity:launch:local failed to construct child list on error %s",
                             ORTE_NAME_PRINT(ORTE_PROC_MY_NAME), ORTE_ERROR_NAME(rc)));
        return rc;
    }

    /* launch the local procs */
    ORTE_ACTIVATE_LOCAL_LAUNCH(job, odls_singularity_fork_local_proc);
...
static int orte_odls_singularity_restart_proc(orte_proc_t *child)
{
    ...
    if (ORTE_SUCCESS != (rc = orte_odls_base_default_restart_proc(child, odls_singularity_fork_local_proc)))
    {
        OPAL_OUTPUT_VERBOSE((2, orte_odls_base_framework.framework_output,
                             "%s odls:singularity:restart_proc failed to launch on error %s",
                             ORTE_NAME_PRINT(ORTE_PROC_MY_NAME), ORTE_ERROR_NAME(rc)));
    }
    return rc;
}
