...
int main(int argc, char *argv[])
{
    ...
    if (ORTE_SUCCESS != opal_init_util(&argc, &argv)) {
        orte_show_help("help-pinfo.txt", "lib-call-fail", true,
    ...
    if( ORTE_SUCCESS != mca_base_open() ) {
        orte_show_help("help-pinfo.txt", "lib-call-fail", true, "mca_base_open", __FILE__, __LINE__ );
    ...
    if (!cmd_error &&
        ...
        str = opal_show_help_string("help-pinfo.txt", "usage", true,
                                    usage);
        if (NULL != str) {
            printf("%s", str);
            free(str);
