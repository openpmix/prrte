...
AC_DEFUN([PRTE_WRAPPER_FLAGS_ADD], [
    m4_ifdef([mca_component_configure_active],
        [m4_fatal([PRTE_WRAPPER_FLAGS_ADD can not be called from a component configure])])
    m4_if([$1], [CPPFLAGS], [PRTE_FLAGS_APPEND_UNIQ([wrapper_extra_cppflags], [$2])],
          [$1], [CFLAGS], [PRTE_FLAGS_APPEND_UNIQ([wrapper_extra_cflags], [$2])],
          [$1], [LDFLAGS], [PRTE_FLAGS_APPEND_UNIQ([wrapper_extra_ldflags], [$2])],
          [$1], [LIBS], [PRTE_FLAGS_APPEND_UNIQ([wrapper_extra_libs], [$2])],
...
# macro works in pair with PRTE_SETUP_WRAPPER_FINAL, which should be called
# almost at the end of configure (after the last call to PRTE_WRAPPER_FLAGS_ADD
...
AC_DEFUN([PRTE_SETUP_WRAPPER_INIT],[
...
# PRTE_LIBTOOL_CONFIG(libtool-variable, result-variable,
#                     libtool-tag, extra-code)
# Retrieve information from the generated libtool
AC_DEFUN([PRTE_LIBTOOL_CONFIG],[
    PRTE_VAR_SCOPE_PUSH([rpath_script rpath_outfile])
...
$PRTE_TOP_BUILDDIR/libtool $3 --config > $rpath_outfile
...
AC_DEFUN([PRTE_SETUP_RPATH],[
    PRTE_VAR_SCOPE_PUSH([rpath_libdir_save])
    AC_MSG_CHECKING([if linker supports RPATH])
    PRTE_LIBTOOL_CONFIG([hardcode_libdir_flag_spec],[rpath_args],[],[libdir=LIBDIR])
...
AC_DEFUN([RPATHIFY_LDFLAGS_INTERNAL],[
    PRTE_VAR_SCOPE_PUSH([rpath_out rpath_dir rpath_tmp])
...
dnl
AC_DEFUN([_PRTE_SETUP_WRAPPER_FINAL_PKGCONFIG],[
...
# ---------------------------
AC_DEFUN([PRTE_SETUP_WRAPPER_FINAL],[
    ...
    AC_MSG_CHECKING([for PRTE CPPFLAGS])
    PRTE_WRAPPER_EXTRA_CPPFLAGS="$wrapper_extra_cppflags $with_wrapper_cppflags"
    AC_SUBST([PRTE_WRAPPER_EXTRA_CPPFLAGS])
    AC_MSG_RESULT([$PRTE_WRAPPER_EXTRA_CPPFLAGS])

    AC_MSG_CHECKING([for PRTE CFLAGS])
    PRTE_WRAPPER_EXTRA_CFLAGS="$wrapper_extra_cflags $with_wrapper_cflags"
    AC_SUBST([PRTE_WRAPPER_EXTRA_CFLAGS])
    AC_MSG_RESULT([$PRTE_WRAPPER_EXTRA_CFLAGS])

    AC_MSG_CHECKING([for PRTE CFLAGS_PREFIX])
    PRTE_WRAPPER_EXTRA_CFLAGS_PREFIX="$with_wrapper_cflags_prefix"
    AC_SUBST([PRTE_WRAPPER_EXTRA_CFLAGS_PREFIX])
    AC_MSG_RESULT([$PRTE_WRAPPER_EXTRA_CFLAGS_PREFIX])

    AC_MSG_CHECKING([for PRTE CXXFLAGS])
    PRTE_WRAPPER_EXTRA_CXXFLAGS="$wrapper_extra_cxxflags $with_wrapper_cxxflags"
    AC_SUBST([PRTE_WRAPPER_EXTRA_CXXFLAGS])
    AC_MSG_RESULT([$PRTE_WRAPPER_EXTRA_CXXFLAGS])

    AC_MSG_CHECKING([for PRTE CXXFLAGS_PREFIX])
    PRTE_WRAPPER_EXTRA_CXXFLAGS_PREFIX="$with_wrapper_cxxflags_prefix"
    AC_SUBST([PRTE_WRAPPER_EXTRA_CXXFLAGS_PREFIX])
    AC_MSG_RESULT([$PRTE_WRAPPER_EXTRA_CXXFLAGS_PREFIX])

    AC_MSG_CHECKING([for PRTE LDFLAGS])
    PRTE_WRAPPER_EXTRA_LDFLAGS="$prte_mca_wrapper_extra_ldflags $wrapper_extra_ldflags $with_wrapper_ldflags"
    RPATHIFY_LDFLAGS([PRTE_WRAPPER_EXTRA_LDFLAGS])
    AC_SUBST([PRTE_WRAPPER_EXTRA_LDFLAGS])
    AC_MSG_RESULT([$PRTE_WRAPPER_EXTRA_LDFLAGS])

    # Convert @{libdir} to ${libdir} for pkg-config
    _PRTE_SETUP_WRAPPER_FINAL_PKGCONFIG([PRTE])
    ...
    AC_MSG_CHECKING([for PRTE LIBS])
    PRTE_WRAPPER_EXTRA_LIBS="$prte_mca_wrapper_extra_libs"
    PRTE_FLAGS_APPEND_UNIQ([PRTE_WRAPPER_EXTRA_LIBS], [$wrapper_extra_libs])
    PRTE_WRAPPER_EXTRA_LIBS="$PRTE_WRAPPER_EXTRA_LIBS $with_wrapper_libs"
    AC_SUBST([PRTE_WRAPPER_EXTRA_LIBS])
    AC_MSG_RESULT([$PRTE_WRAPPER_EXTRA_LIBS])
