...
AC_DEFUN([PRTE_CASE_SENSITIVE_FS_SETUP],[
#
# Arguments: none
#
# Dependencies: None
...
# (like HFS+ on MacOS X and MS Windows) are not case sensitive - prtecc
# and prteCC are the same file.
#
...
if test "$with_cs_fs" = "yes"; then
    ...
    PRTE_WANT_CS_FS=$have_cs_fs
else
    PRTE_WANT_CS_FS=0
fi

if test "$PRTE_WANT_CS_FS" = "1"; then
    AC_MSG_RESULT([yes])
else
    AC_MSG_RESULT([no])
fi

AM_CONDITIONAL(CASE_SENSITIVE_FS, test "$PRTE_WANT_CS_FS" = "1")

if test "$PRTE_WANT_CS_FS" = "0"; then
	cat <<EOF
...
Because PRTE is being installed on a non-case sensitive file
system, the C++ wrapper compiler will be named prtec++ instead of the
traditional prteCC.
