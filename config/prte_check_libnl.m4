...
dnl
dnl _PRTE_CHECK_PACKAGE_LIB() invokes PRTE_LIBNL_SANITY_CHECK() in order
...
dnl PRTE_LIBNL_SANITY_INIT()
dnl --------------------------------------------------------------------
AC_DEFUN([PRTE_LIBNL_SANITY_INIT], [
    prte_libnl_version=0
    prte_libnlv1_libs=
    prte_libnlv3_libs=
    AC_ARG_WITH([libnl],
    ...
    case "$with_libnl" in
        ...
        *)
            ...
            prte_want_libnl=yes
            prte_libnl_location=$with_libnl
            ;;
    esac
])

dnl PRTE_LIBNL_SANITY_FAIL_MSG(lib)
dnl
dnl Helper to pring a big warning message when we detect a libnl conflict.
dnl
dnl --------------------------------------------------------------------
AC_DEFUN([PRTE_LIBNL_SANITY_FAIL_MSG], [
...
dnl PRTE_LIBNL_SANITY_CHECK(lib, function, LIBS, libnl_check_ok)
dnl
dnl This macro is invoked from PRTE_CHECK_PACKAGE to make sure that
...
dnl that pulls in libnl v3, if PRTE_CHECK_PACKAGE is later called that
...
dnl
dnl --------------------------------------------------------------------
AC_DEFUN([PRTE_LIBNL_SANITY_CHECK], [
    PRTE_VAR_SCOPE_PUSH([prte_libnl_sane])
    prte_libnl_sane=1
    case $host in
        *linux*)
            PRTE_LIBNL_SANITY_CHECK_LINUX($1, $2, $3, prte_libnl_sane)
            ;;
    esac

    $4=$prte_libnl_sane
    PRTE_VAR_SCOPE_POP([prte_libnl_sane])
])

dnl
dnl Simple helper for PRTE_LIBNL_SANITY_CHECK
dnl $1: library name
dnl $2: function
dnl $3: LIBS
dnl $4: output variable (1=ok, 0=not ok)
dnl
AC_DEFUN([PRTE_LIBNL_SANITY_CHECK_LINUX], [
    PRTE_VAR_SCOPE_PUSH([this_requires_v1 libnl_sane this_requires_v3 ldd_output result_msg])
...
EOF
    ...
    this_requires_v3=0
    result_msg=
    PRTE_LOG_COMMAND([$CC -o conftest $CFLAGS $CPPFLAGS conftest_c.$ac_ext $LDFLAGS -l$1 $LIBS $3],
    ...
    AS_IF([test $this_requires_v1 -eq 1 && test $this_requires_v3 -eq 1],
          [AC_MSG_WARN([Unfortunately, lib$1 links to both libnl and libnl-3.])
           PRTE_LIBNL_SANITY_FAIL_MSG($1)
           libnl_sane=0])

    # Does this library require v1, but some prior library required
    # v3?  If so, fail.
    AS_IF([test $libnl_sane -eq 1 && test $this_requires_v1 -eq 1],
          [AS_IF([test $prte_libnl_version -eq 3],
                 [AC_MSG_WARN([libnl version conflict: $prte_libnlv3_libs requires libnl-3 whereas $1 requires libnl])
                  PRTE_LIBNL_SANITY_FAIL_MSG($1)
                  libnl_sane=0],
                 [prte_libnlv1_libs="$prte_libnlv1_libs $1"
                  PRTE_UNIQ([prte_libnlv1_libs])
                  prte_libnl_version=1])
           ])
    ...
    AS_IF([test $libnl_sane -eq 1 && test $this_requires_v3 -eq 1],
          [AS_IF([test $prte_libnl_version -eq 1],
                 [AC_MSG_WARN([libnl version conflict: $prte_libnlv1_libs requires libnl whereas lib$1 requires libnl-3])
                  PRTE_LIBNL_SANITY_FAIL_MSG($1)
                  libnl_sane=0],
                 [prte_libnlv3_libs="$prte_libnlv3_libs $1"
                  PRTE_UNIQ([prte_libnlv3_libs])
                  prte_libnl_version=3])
          ])
    ...
    PRTE_VAR_SCOPE_POP([ldd_output libnl_sane this_requires_v1 this_requires_v3 result_msg])
])
...
dnl - Set PRTE_HAVE_LIBNL3 1 if libnl-3 will be used
dnl
AC_DEFUN([PRTE_CHECK_LIBNL_V3],[
    PRTE_VAR_SCOPE_PUSH([CPPFLAGS_save prte_tmp_CPPFLAGS LIBS_save LDFLAGS_save])
    AC_MSG_NOTICE([checking for libnl v3])

    AS_IF([test "$prte_want_libnl" != "no"],
          [AS_IF([test -z "$prte_libnl_location"],
                 [AC_MSG_CHECKING([for /usr/include/libnl3])
                  AS_IF([test -d "/usr/include/libnl3"],
                        [prte_tmp_CPPFLAGS=-I/usr/include/libnl3
                         prte_libnlv3_happy=1
                         AC_MSG_RESULT([found])],
                        [AC_MSG_RESULT([not found])
                         AC_MSG_CHECKING([for /usr/local/include/libnl3])
                         AS_IF([test -d "/usr/local/include/libnl3"],
                               [prte_tmp_CPPFLAGS=-I/usr/local/include/netlink3
                                prte_libnlv3_happy=1
                                AC_MSG_RESULT([found])],
                               [prte_libnlv3_happy=0
                                AC_MSG_RESULT([not found])])])],
                 [AC_MSG_CHECKING([for $1/include/libnl3])
                  AS_IF([test -d "$1/include/libnl3"],
                        [prte_tmp_CPPFLAGS="-I$1/include/libnl3"
                         prte_libnlv3_happy=1
                         AC_MSG_RESULT([found])],
                        [prte_libnlv3_happy=0
                         AC_MSG_RESULT([not found])])])
           CPPFLAGS_save=$CPPFLAGS
           CPPFLAGS="$prte_tmp_CPPFLAGS $CPPFLAGS"

           # Random note: netlink/version.h is only in libnl v3 - it is not in libnl v1.
           # Also, nl_recvmsgs_report is only in libnl v3.
           AS_IF([test $prte_libnlv3_happy -eq 1],
                 [PRTE_CHECK_PACKAGE([$2],
                                     [netlink/version.h],
                                     [nl-3],
                                     [nl_recvmsgs_report],
                                     [],
                  ...
                  # Note that PRTE_CHECK_PACKAGE is going to add
...
#error "LIBNL_VER_MAJ != 3, I am sad"
    ...
    AS_IF([test $prte_libnlv3_happy -eq 1],
          [$2_LIBS="-lnl-3 -lnl-route-3"
           PRTE_HAVE_LIBNL3=1],
          [# PRTE_CHECK_PACKAGE(...,nl_recvmsgs_report,...) might have set the variables below
...
dnl - Set PRTE_HAVE_LIBNL3 0 if libnl will be used
dnl
AC_DEFUN([PRTE_CHECK_LIBNL_V1],[
    ...
    AS_IF([test "$prte_want_libnl" != "no"],
          [PRTE_CHECK_PACKAGE([$2],
                              ...
                              [$1],
                              [],
                              [prte_libnlv1_happy=1],
                              [prte_libnlv1_happy=0])],
          [prte_libnlv1_happy=0])

    AS_IF([test $prte_libnlv1_happy -eq 1],
          [$2_LIBS="-lnl -lm"
           PRTE_HAVE_LIBNL3=0])
])
...
dnl
AC_DEFUN([PRTE_CHECK_LIBNL_SUMMARY],[
    AC_MSG_CHECKING([for libraries that use libnl v1])
    AS_IF([test -n "$prte_libnlv1_libs"],
          [AC_MSG_RESULT([$prte_libnlv1_libs])],
          [AC_MSG_RESULT([(none)])])
    AC_MSG_CHECKING([for libraries that use libnl v3])
    AS_IF([test -n "$prte_libnlv3_libs"],
          [AC_MSG_RESULT([$prte_libnlv3_libs])],
          [AC_MSG_RESULT([(none)])])
