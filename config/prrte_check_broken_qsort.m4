...
dnl the prrte_qsort instead.

# check for broken qsort
# PRRTE_CHECK_BROKEN_QSORT(prefix, [action-if-found], [action-if-not-found])
# --------------------------------------------------------
AC_DEFUN([PRRTE_CHECK_BROKEN_QSORT],[
  ...
  AC_DEFINE_UNQUOTED([PRRTE_HAVE_BROKEN_QSORT], [$define_result],
