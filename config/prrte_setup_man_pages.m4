...
AC_DEFUN([PRRTE_SETUP_MAN_PAGES],[
    AC_ARG_ENABLE(man-pages,
                  [AC_HELP_STRING([--disable-man-pages],
                                  [Do not generate/install man pages (default: enable)])])

    PANDOC=
    PRRTE_ENABLE_MAN_PAGES=0
    AC_MSG_CHECKING([if want man pages])
    AS_IF([test -z "$enable_man_pages" || test "$enable_man_pages" = "yes"],
          [AC_MSG_RESULT([yes])
	   PRRTE_ENABLE_MAN_PAGES=1
	   _PRRTE_SETUP_PANDOC],
	  [AC_MSG_RESULT([no])])

    AC_SUBST(PANDOC)
    AM_CONDITIONAL([PRRTE_ENABLE_MAN_PAGES], [test $PRRTE_ENABLE_MAN_PAGES -eq 1])
])

dnl Back-end pandoc setup
AC_DEFUN([_PRRTE_SETUP_PANDOC],[
    PRRTE_VAR_SCOPE_PUSH([min_major_version min_minor_version pandoc_version pandoc_major pandoc_minor])
    ...
    AS_IF([test -z "$PANDOC" || test -n "`echo $PANDOC | $GREP missing`"],
          [AS_IF([test ! -f "$srcdir/tools/wrapper/prrte_wrapper.1"],
                 [AC_MSG_WARN([*** Could not find a suitable pandoc on your system.])
                  ...
                  AC_MSG_WARN([*** NOTE: If you are building from a tarball downloaded from the PRRTE web site, you do not need Pandoc])
