...
AC_DEFUN([PRRTE_CHECK_LSF],[
    AS_IF([test -z "$prrte_check_lsf_happy"],[
       ...
       AS_IF([test "$with_lsf" != "no"],[
          ...
          prrte_check_lsf_event_conflict=na
          # Split libs into an array, see if -levent is in that list
          prrte_check_lsf_libevent_present=`echo $LIBS | awk '{split([$]0, a, " "); {for (k in a) {if (a[[k]] == "-levent") {print a[[k]]}}}}' | wc -l`
          AS_IF([test "$prrte_check_lsf_happy" = "no"],
                [AS_IF([test "$prrte_check_lsf_libevent_present" != 0],
                       [AS_IF([test "$prrte_check_lsf_libdir" = "" ],
                              [],
                              [LDFLAGS="$LDFLAGS -L$prrte_check_lsf_libdir"])
                        ...
                        AC_CHECK_LIB([event], [evthread_set_condition_callbacks],
                                     [AC_MSG_CHECKING([for libevent conflict])
                                      ...
                                      prrte_check_lsf_event_conflict=no],
                                     [AC_MSG_CHECKING([for libevent conflict])
                                      AC_MSG_RESULT([Yes. A wrong libevent.so was linked.])
                                      prrte_check_lsf_event_conflict=yes])
                       ],
                       [AC_MSG_CHECKING([for libevent conflict])
          ...
          AS_IF([test "$prrte_check_lsf_event_conflict" = "yes"],
                [AC_MSG_WARN([===================================================================])
