...
int main(int argc, char *argv[])
{
    ...
    (void) pmix_mca_base_var_register("prte", "prte", NULL, "parent_uri",
                                      ...
                                      PMIX_MCA_BASE_VAR_TYPE_STRING, &prte_parent_uri);
    ...
    if (pmix_cmd_line_is_taken(&results, PRTE_CLI_TREE_SPAWN)) {
        ...
        opt = pmix_cmd_line_get_param(&results, PMIX_CLI_PMIXMCA);
        if (NULL != opt) {
            ...
            for (i=0; NULL != opt->values[i]; i++) {
                ...
                *t = '\0';
                ++t;
                pmix_argv_append_nosize(&prted_cmd_line, "--"PMIX_CLI_PMIXMCA);
                pmix_argv_append_nosize(&prted_cmd_line, opt->values[i]);
                pmix_argv_append_nosize(&prted_cmd_line, t);
                --t;
                *t = '=';
