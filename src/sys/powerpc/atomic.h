...
#if (PRTE_ASSEMBLY_ARCH == PRTE_POWERPC64) || PRTE_ASM_SUPPORT_64BIT
...
#endif /* end PRTE_GCC_INLINE_ASSEMBLY */
...
#define PRTE_ASM_ADDR(a) ((uintptr_t)a)
#else
#define PRTE_ASM_ADDR(a) (a)
#endif
...
#define PRTE_ASM_VALUE64(x) (void *)(intptr_t) (x)
#else
#define PRTE_ASM_VALUE64(x) x
#endif
...
static inline bool prte_atomic_compare_exchange_strong_32 (prte_atomic_int32_t *addr, int32_t *oldval, int32_t newval)
{
    ...
    __asm__ __volatile__ (
                          ...
                          "2:"
                          : "=&r" (prev), "=m" (*addr)
                          : "r" PRTE_ASM_ADDR(addr), "r" (*oldval), "r" (newval), "m" (*addr)
                          : "cc", "memory");
...
#define prte_atomic_sc_32(addr, value, ret)                             \
    do {                                                                \
        prte_atomic_int32_t *_addr = (addr);                               \
...
#if (PRTE_ASSEMBLY_ARCH == PRTE_POWERPC64)

#if  PRTE_GCC_INLINE_ASSEMBLY

#define PRTE_ATOMIC_POWERPC_DEFINE_ATOMIC_64(type, instr)               \
static inline int64_t prte_atomic_fetch_ ## type ## _64(prte_atomic_int64_t* v, int64_t val) \
{                                                                       \
    int64_t t, old;                                                     \
                                                                        \
    __asm__ __volatile__(                                               \
                         ...
                         "     bne-    1b           \n\t"               \
                         : "=&r" (t), "=&r" (old), "=m" (*v)            \
                         : "r" (PRTE_ASM_VALUE64(val)), "r" PRTE_ASM_ADDR(v), "m" (*v) \
                         : "cc");                                       \
                                                                        \
    return old;                                                         \
}

PRTE_ATOMIC_POWERPC_DEFINE_ATOMIC_64(add, add)
PRTE_ATOMIC_POWERPC_DEFINE_ATOMIC_64(and, and)
PRTE_ATOMIC_POWERPC_DEFINE_ATOMIC_64(or, or)
PRTE_ATOMIC_POWERPC_DEFINE_ATOMIC_64(xor, xor)
PRTE_ATOMIC_POWERPC_DEFINE_ATOMIC_64(sub, subf)
...
static inline bool prte_atomic_compare_exchange_strong_64 (prte_atomic_int64_t *addr, int64_t *oldval, int64_t newval)
{
    ...
    __asm__ __volatile__ (
                          ...
                          "2:"
                          : "=&r" (prev), "=m" (*addr)
                          : "r" (addr), "r" (PRTE_ASM_VALUE64(*oldval)), "r" (PRTE_ASM_VALUE64(newval)), "m" (*addr)
                          : "cc", "memory");
...
#define prte_atomic_sc_64(addr, value, ret)                             \
    do {                                                                \
        prte_atomic_int64_t *_addr = (addr);                               \
        ...
        __asm__ __volatile__ ("   stdcx.  %2, 0, %1  \n\t"              \
                              ...
                              "1:"                                      \
                              : "=r" (_ret)                             \
                              : "r" (_addr), "r" (PRTE_ASM_VALUE64(_newval)) \
                              : "cc", "memory");                        \
        ret = _ret;                                                     \
    } while (0)
...
static inline int64_t prte_atomic_swap_64(prte_atomic_int64_t *addr, int64_t newval)
{
   ...
   __asm__ __volatile__ ("1: ldarx   %0, 0, %2  \n\t"
                         ...
                         "   bne-    1b         \n\t"
                         : "=&r" (ret), "=m" (*addr)
                         : "r" (addr), "r" (PRTE_ASM_VALUE64(newval))
                         : "cc", "memory");
...
#elif (PRTE_ASSEMBLY_ARCH == PRTE_POWERPC32) && PRTE_ASM_SUPPORT_64BIT
...
#define PRTE_ATOMIC_POWERPC_DEFINE_ATOMIC_32(type, instr)               \
static inline int32_t prte_atomic_fetch_ ## type ## _32(prte_atomic_int32_t* v, int val) \
{                                                                       \
    int32_t t, old;                                                     \
                                                                        \
    __asm__ __volatile__(                                               \
                         ...
                         "     bne-    1b           \n\t"               \
                         : "=&r" (t), "=&r" (old), "=m" (*v)            \
                         : "r" (val), "r" PRTE_ASM_ADDR(v), "m" (*v)    \
                         : "cc");                                       \
                                                                        \
    return old;                                                         \
}

PRTE_ATOMIC_POWERPC_DEFINE_ATOMIC_32(add, add)
PRTE_ATOMIC_POWERPC_DEFINE_ATOMIC_32(and, and)
PRTE_ATOMIC_POWERPC_DEFINE_ATOMIC_32(or, or)
PRTE_ATOMIC_POWERPC_DEFINE_ATOMIC_32(xor, xor)
PRTE_ATOMIC_POWERPC_DEFINE_ATOMIC_32(sub, subf)
