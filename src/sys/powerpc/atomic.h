...
#if (PRRTE_ASSEMBLY_ARCH == PRRTE_POWERPC64) || PRRTE_ASM_SUPPORT_64BIT
...
#endif /* end PRRTE_GCC_INLINE_ASSEMBLY */
...
#define PRRTE_ASM_ADDR(a) ((uintptr_t)a)
#else
#define PRRTE_ASM_ADDR(a) (a)
#endif
...
#define PRRTE_ASM_VALUE64(x) (void *)(intptr_t) (x)
#else
#define PRRTE_ASM_VALUE64(x) x
#endif
...
static inline bool prrte_atomic_compare_exchange_strong_32 (prrte_atomic_int32_t *addr, int32_t *oldval, int32_t newval)
{
    ...
    __asm__ __volatile__ (
                          ...
                          "2:"
                          : "=&r" (prev), "=m" (*addr)
                          : "r" PRRTE_ASM_ADDR(addr), "r" (*oldval), "r" (newval), "m" (*addr)
                          : "cc", "memory");
...
#define prrte_atomic_sc_32(addr, value, ret)                             \
    do {                                                                \
        prrte_atomic_int32_t *_addr = (addr);                               \
...
#if (PRRTE_ASSEMBLY_ARCH == PRRTE_POWERPC64)

#if  PRRTE_GCC_INLINE_ASSEMBLY

#define PRRTE_ATOMIC_POWERPC_DEFINE_ATOMIC_64(type, instr)               \
static inline int64_t prrte_atomic_fetch_ ## type ## _64(prrte_atomic_int64_t* v, int64_t val) \
{                                                                       \
    int64_t t, old;                                                     \
                                                                        \
    __asm__ __volatile__(                                               \
                         ...
                         "     bne-    1b           \n\t"               \
                         : "=&r" (t), "=&r" (old), "=m" (*v)            \
                         : "r" (PRRTE_ASM_VALUE64(val)), "r" PRRTE_ASM_ADDR(v), "m" (*v) \
                         : "cc");                                       \
                                                                        \
    return old;                                                         \
}

PRRTE_ATOMIC_POWERPC_DEFINE_ATOMIC_64(add, add)
PRRTE_ATOMIC_POWERPC_DEFINE_ATOMIC_64(and, and)
PRRTE_ATOMIC_POWERPC_DEFINE_ATOMIC_64(or, or)
PRRTE_ATOMIC_POWERPC_DEFINE_ATOMIC_64(xor, xor)
PRRTE_ATOMIC_POWERPC_DEFINE_ATOMIC_64(sub, subf)
...
static inline bool prrte_atomic_compare_exchange_strong_64 (prrte_atomic_int64_t *addr, int64_t *oldval, int64_t newval)
{
    ...
    __asm__ __volatile__ (
                          ...
                          "2:"
                          : "=&r" (prev), "=m" (*addr)
                          : "r" (addr), "r" (PRRTE_ASM_VALUE64(*oldval)), "r" (PRRTE_ASM_VALUE64(newval)), "m" (*addr)
                          : "cc", "memory");
...
#define prrte_atomic_sc_64(addr, value, ret)                             \
    do {                                                                \
        prrte_atomic_int64_t *_addr = (addr);                               \
        ...
        __asm__ __volatile__ ("   stdcx.  %2, 0, %1  \n\t"              \
                              ...
                              "1:"                                      \
                              : "=r" (_ret)                             \
                              : "r" (_addr), "r" (PRRTE_ASM_VALUE64(_newval)) \
                              : "cc", "memory");                        \
        ret = _ret;                                                     \
    } while (0)
...
static inline int64_t prrte_atomic_swap_64(prrte_atomic_int64_t *addr, int64_t newval)
{
   ...
   __asm__ __volatile__ ("1: ldarx   %0, 0, %2  \n\t"
                         ...
                         "   bne-    1b         \n\t"
                         : "=&r" (ret), "=m" (*addr)
                         : "r" (addr), "r" (PRRTE_ASM_VALUE64(newval))
                         : "cc", "memory");
...
#elif (PRRTE_ASSEMBLY_ARCH == PRRTE_POWERPC32) && PRRTE_ASM_SUPPORT_64BIT
...
#define PRRTE_ATOMIC_POWERPC_DEFINE_ATOMIC_32(type, instr)               \
static inline int32_t prrte_atomic_fetch_ ## type ## _32(prrte_atomic_int32_t* v, int val) \
{                                                                       \
    int32_t t, old;                                                     \
                                                                        \
    __asm__ __volatile__(                                               \
                         ...
                         "     bne-    1b           \n\t"               \
                         : "=&r" (t), "=&r" (old), "=m" (*v)            \
                         : "r" (val), "r" PRRTE_ASM_ADDR(v), "m" (*v)    \
                         : "cc");                                       \
                                                                        \
    return old;                                                         \
}

PRRTE_ATOMIC_POWERPC_DEFINE_ATOMIC_32(add, add)
PRRTE_ATOMIC_POWERPC_DEFINE_ATOMIC_32(and, and)
PRRTE_ATOMIC_POWERPC_DEFINE_ATOMIC_32(or, or)
PRRTE_ATOMIC_POWERPC_DEFINE_ATOMIC_32(xor, xor)
PRRTE_ATOMIC_POWERPC_DEFINE_ATOMIC_32(sub, subf)
