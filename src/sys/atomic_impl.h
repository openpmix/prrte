...
#    define PRTE_ATOMIC_DEFINE_CMPXCG_OP(type, bits, operation, name)                              \
        ...
        {                                                                                          \
            ...
            do {                                                                                   \
                oldval = *addr;                                                                    \
            } while (!prte_atomic_compare_exchange_strong_##bits(addr, &oldval,                    \
...
#        define PRTE_ATOMIC_DEFINE_CMPXCG_XX(semantics)                                            \
            static inline bool prte_atomic_compare_exchange_strong##semantics##xx(                 \
                prte_atomic_intptr_t *addr, intptr_t *oldval, int64_t newval, const size_t length) \
            {                                                                                      \
                switch (length) {                                                                  \
                case 4:                                                                            \
...
#        define PRTE_ATOMIC_DEFINE_CMPXCG_PTR_XX(semantics)                                     \
            static inline bool                                                                  \
                prte_atomic_compare_exchange_strong##semantics##ptr(prte_atomic_intptr_t *addr, \
                                                                    intptr_t *oldval,           \
                                                                    intptr_t newval)            \
            {                                                                                   \
                return prte_atomic_compare_exchange_strong_32((prte_atomic_int32_t *) addr,     \
...
#        define prte_atomic_swap_ptr(addr, value) \
            (intptr_t) prte_atomic_swap_32((prte_atomic_int32_t *) addr, (int32_t) value)
#    elif SIZEOF_VOID_P == 8 && PRTE_HAVE_ATOMIC_SWAP_64
#        define prte_atomic_swap_ptr(addr, value) \
            (intptr_t) prte_atomic_swap_64((prte_atomic_int64_t *) addr, (int64_t) value)
#    endif
...
#        define prte_atomic_sc_ptr(addr, value, ret) \
            prte_atomic_sc_32((prte_atomic_int32_t *) (addr), (intptr_t)(value), ret)
...
#        define prte_atomic_sc_ptr(addr, value, ret) \
            prte_atomic_sc_64((prte_atomic_int64_t *) (addr), (intptr_t)(value), ret)
...
#    define PRTE_ATOMIC_DEFINE_OP_FETCH(op, operation, type, ptr_type, suffix)             \
        static inline type prte_atomic_##op##_fetch_##suffix(prte_atomic_##ptr_type *addr, \
                                                             type value)                   \
        {                                                                                  \
...
static inline int prte_atomic_trylock(prte_atomic_lock_t *lock)
{
    ...
    bool ret = prte_atomic_compare_exchange_strong_acq_32(&lock->u.lock, &unlocked,
                                                          PRTE_ATOMIC_LOCK_LOCKED);
