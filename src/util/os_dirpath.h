 ...
 *
 * The prrte_os_dirpath_create() function creates a directory
 ...
 * prrte_os_dirpath_create() function will check to ensure that
 ...
 * permissions. The function will return PRRTE_ERROR if this cannot
 * be done.
 **/

#ifndef PRRTE_OS_DIRPATH_CREATE_H
#define PRRTE_OS_DIRPATH_CREATE_H
 ...
 * @retval PRRTE_SUCCESS If the directory tree has been successfully created with
 * the specified access permissions.
 * @retval PRRTE_ERROR If the directory tree could not be created with the
 * specified access permissions.
 */

PRRTE_EXPORT int prrte_os_dirpath_create(const char *path, const mode_t mode);
 ...
 */
PRRTE_EXPORT bool prrte_os_dirpath_is_empty(const char *path);
 ...
 *
 * @retval PRRTE_SUCCESS If directory exists, and permissions match
 * @retval PRRTE_ERR_NOT_FOUND If directory does not exist
 * @retval PRRTE_ERROR   If directory exists, and permissions do not match
 */
PRRTE_EXPORT int prrte_os_dirpath_access(const char *path, const mode_t mode );

/**
 * Callback for prrte_os_dirpath_destroy(). Call for every file/directory before
 ...
 */
typedef bool (*prrte_os_dirpath_destroy_callback_fn_t)(const char *root, const char *path);
 ...
 *
 * @retval PRRTE_SUCCESS If the directory was successfully removed or removed to the
 *                      specification of the user (i.e., obeyed the callback function).
 * @retval PRRTE_ERR_NOT_FOUND If directory does not exist.
 * @retval PRRTE_ERROR If the directory cannnot be removed, accessed properly, or contains
 *                    directories that could not be removed..
 */
PRRTE_EXPORT int prrte_os_dirpath_destroy(const char *path,
                                          bool recursive,
