...
void prte_string_copy(char *dest, const char *src, size_t dest_len)
{
    ...
    // PRTE does not do *giant* string copies.  Hence, we use the
    ...
    // prte_string_copy() more than 128K), the core dumps that are
    // generated by the assert() failure should make this fairly
    // obvious.
    assert(dest_len <= PRTE_MAX_SIZE_ALLOWED_BY_PRTE_STRING_COPY);
