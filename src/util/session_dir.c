...
static int prte_create_dir(char *directory)
{
    ...
    if( PRTE_ERR_NOT_FOUND !=
        (ret = prte_os_dirpath_access(directory, my_mode)) ) {
...
int prte_session_setup_base(prte_process_name_t *proc)
{
     ...
     */
    if (NULL != prte_prohibited_session_dirs ||
            NULL != prte_process_info.tmpdir_base ) {
        char **list;
        int i, len;
        ...
        for (i=0; i < len; i++) {
            ...
            if (0 == strncmp(prte_process_info.tmpdir_base, list[i], strlen(list[i]))) {
                ...
                prte_show_help("help-prte-runtime.txt",
                               ...
                               true, prte_process_info.tmpdir_base,
...
int prte_session_dir(bool create, prte_process_name_t *proc)
{
    ...
    if (prte_debug_flag) {
        prte_output(0, "procdir: %s",
                    PRTE_PRINTF_FIX_STRING(prte_process_info.proc_session_dir));
        prte_output(0, "jobdir: %s",
                    PRTE_PRINTF_FIX_STRING(prte_process_info.job_session_dir));
        prte_output(0, "job: %s",
                    PRTE_PRINTF_FIX_STRING(prte_process_info.jobfam_session_dir));
        prte_output(0, "top: %s",
                    PRTE_PRINTF_FIX_STRING(prte_process_info.top_session_dir));
        prte_output(0, "tmp: %s",
                    PRTE_PRINTF_FIX_STRING(prte_process_info.tmpdir_base));
    }
...
prte_session_dir_cleanup(prte_jobid_t jobid)
{
     ...
     */
    prte_os_dirpath_destroy(prte_process_info.jobfam_session_dir,
                            true, prte_dir_check_file);
...
prte_session_dir_finalize(prte_process_name_t *proc)
{
    ...
    prte_os_dirpath_destroy(prte_process_info.proc_session_dir,
                            false, prte_dir_check_file);
    ...
    prte_os_dirpath_destroy(prte_process_info.job_session_dir,
                            false, prte_dir_check_file);
...
static bool
prte_dir_check_file(const char *root, const char *path)
{
    struct stat st;
    char *fullpath;
