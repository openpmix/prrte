...
static PRTE_CLASS_INSTANCE(prte_data_object_t,
                          prte_object_t,
                          construct, destruct);
...
{
    prte_argv_free(p->keys);
    PRTE_LIST_DESTRUCT(&p->answers);
}
static PRTE_CLASS_INSTANCE(prte_data_req_t,
                          prte_list_item_t,
                          rqcon, rqdes);
...
int prte_data_server_init(void)
{
    ...
    if (0 <= prte_data_server_verbosity) {
        ...
        prte_output_set_verbosity(prte_data_server_output,
                                  prte_data_server_verbosity);
    }

    PRTE_CONSTRUCT(&prte_data_server_store, prte_pointer_array_t);
    if (PRTE_SUCCESS != (rc = prte_pointer_array_init(&prte_data_server_store,
                                                      1,
                                                      INT_MAX,
                                                      1))) {
        PRTE_ERROR_LOG(rc);
...
void prte_data_server_finalize(void)
{
    ...
    for (i=0; i < prte_data_server_store.size; i++) {
        if (NULL != (data = (prte_data_object_t*)prte_pointer_array_get_item(&prte_data_server_store, i))) {
            PRTE_RELEASE(data);
        }
    }
                      ...
                      void* cbdata)
{
    ...
    count = 1;
    rc = PMIx_Data_unpack(NULL, buffer, &boptr, &count, PMIX_BYTE_OBJECT);
    if (PMIX_SUCCESS != rc) {
        PMIX_ERROR_LOG(rc);
        goto SEND_ERROR;
    }
    ...
    case PRTE_PMIX_PUBLISH_CMD:
        ...
        count = 1;
        if (PMIX_SUCCESS != (ret = PMIx_Data_unpack(NULL, &pbkt, &data->owner, &count, PMIX_PROC))) {
            PMIX_ERROR_LOG(ret);
        ...
        count = 1;
        if (PMIX_SUCCESS != (ret = PMIx_Data_unpack(NULL, &pbkt, &data->ninfo, &count, PMIX_SIZE))) {
            PMIX_ERROR_LOG(ret);
            PMIX_DATA_BUFFER_DESTRUCT(&pbkt);
            PRTE_RELEASE(data);
            rc = PRTE_ERR_UNPACK_FAILURE;
        ...
        /* unpack into it */
        count = data->ninfo;
        if (PMIX_SUCCESS != (ret = PMIx_Data_unpack(NULL, &pbkt, data->info, &count, PMIX_INFO))) {
            PMIX_ERROR_LOG(ret);
            PMIX_DATA_BUFFER_DESTRUCT(&pbkt);
            PRTE_RELEASE(data);
            rc = PRTE_ERR_UNPACK_FAILURE;
        ...
        PRTE_LIST_FOREACH_SAFE(req, rqnext, &pending, prte_data_req_t) {
            ...
            if (0 < (n = prte_list_get_size(&req->answers))) {
                ...
                while (NULL != (rinfo = (prte_ds_info_t*)prte_list_remove_first(&req->answers))) {
                    /* pack the data owner */
                    if (PMIX_SUCCESS != (ret = PMIx_Data_pack(NULL, &pbkt, &rinfo->source, 1, PMIX_PROC))) {
                        PMIX_ERROR_LOG(ret);
                        PMIX_DATA_BUFFER_DESTRUCT(&pbkt);
                        rc = PRTE_ERR_PACK_FAILURE;
                        goto SEND_ERROR;
                    ...
                    /* pack the data */
                    if (PMIX_SUCCESS != (ret = PMIx_Data_pack(NULL, &pbkt, rinfo->info, 1, PMIX_INFO))) {
                        PMIX_ERROR_LOG(ret);
                        PMIX_DATA_BUFFER_DESTRUCT(&pbkt);
                        rc = PRTE_ERR_PACK_FAILURE;
                        goto SEND_ERROR;
    ...
    case PRTE_PMIX_LOOKUP_CMD:
        ...
        count = 1;
        if (PMIX_SUCCESS != (ret = PMIx_Data_unpack(NULL, &pbkt, &requestor, &count, PMIX_PROC))) {
            PMIX_ERROR_LOG(ret);
            PMIX_DATA_BUFFER_DESTRUCT(&pbkt);
            rc = PRTE_ERR_UNPACK_FAILURE;
            goto SEND_ERROR;
        ...
        count = 1;
        if (PMIX_SUCCESS != (ret = PMIx_Data_unpack(NULL, &pbkt, &ninfo, &count, PMIX_SIZE))) {
            PMIX_ERROR_LOG(ret);
            PMIX_DATA_BUFFER_DESTRUCT(&pbkt);
            rc = PRTE_ERR_UNPACK_FAILURE;
            goto SEND_ERROR;
        ...
        for (n=0; n < ninfo; n++) {
            count = 1;
            if (PMIX_SUCCESS != (ret = PMIx_Data_unpack(NULL, &pbkt, &str, &count, PMIX_STRING))) {
                PMIX_ERROR_LOG(ret);
                PMIX_DATA_BUFFER_DESTRUCT(&pbkt);
                rc = PRTE_ERR_UNPACK_FAILURE;
                prte_argv_free(keys);
            ...
            prte_argv_append_nosize(&keys, str);
            free(str);
        }
        ...
        if (0 < ninfo) {
            ...
            count = ninfo;
            if (PMIX_SUCCESS != (ret = PMIx_Data_unpack(NULL, &pbkt, info, &count, PMIX_INFO))) {
                PMIX_ERROR_LOG(ret);
                PMIX_DATA_BUFFER_DESTRUCT(&pbkt);
        ...
        if (nanswers == (size_t)prte_argv_count(keys)) {
            rc = PRTE_SUCCESS;
        } else {
            prte_output_verbose(1, prte_data_server_output,
                                "%s data server:lookup: at least some data not found %d vs %d",
                                PRTE_NAME_PRINT(PRTE_PROC_MY_NAME), (int)nanswers, (int)prte_argv_count(keys));
        ...
        /* pack it into our reply */
        rc = PMIx_Data_pack(NULL, answer, &bo, 1, PMIX_BYTE_OBJECT);
        if (PMIX_SUCCESS != rc) {
            PMIX_ERROR_LOG(rc);
            PMIX_DATA_BUFFER_RELEASE(answer);
            free(bo.bytes);
