...
prrte_cmd_options_t prrte_cmd_options = {0};
prrte_cmd_line_t *prrte_cmd_line = NULL;
...
prrte_cmd_line_init_t prrte_cmd_line_opts[] = {
    ...
    { NULL, 'h', NULL, "help", 0,
      &prted_globals.help, PRRTE_CMD_LINE_TYPE_BOOL,
      "This help message" },

    { NULL, '\0', NULL, "test-suicide", 1,
      &prted_globals.test_suicide, PRRTE_CMD_LINE_TYPE_BOOL,
      "Suicide instead of clean abort after delay" },

    { "prrte_debug", 'd', NULL, "debug", 0,
      NULL, PRRTE_CMD_LINE_TYPE_BOOL,
      "Debug the OpenRTE" },

    { "prrte_daemonize", '\0', NULL, "daemonize", 0,
      &prted_globals.daemonize, PRRTE_CMD_LINE_TYPE_BOOL,
      "Daemonize the orted into the background" },

    { "prrte_debug_daemons", '\0', NULL, "debug-daemons", 0,
      &prted_globals.debug, PRRTE_CMD_LINE_TYPE_BOOL,
      "Enable debugging of OpenRTE daemons" },

    { "prrte_debug_daemons_file", '\0', NULL, "debug-daemons-file", 0,
      NULL, PRRTE_CMD_LINE_TYPE_BOOL,
      "Enable debugging of OpenRTE daemons, storing output in files" },

    { "prrte_hnp_uri", '\0', NULL, "hnp-uri", 1,
      NULL, PRRTE_CMD_LINE_TYPE_STRING,
      "URI for the HNP"},

    { "prrte_parent_uri", '\0', NULL, "parent-uri", 1,
      NULL, PRRTE_CMD_LINE_TYPE_STRING,
      "URI for the parent if tree launch is enabled."},

    { NULL, '\0', NULL, "set-sid", 0,
      &prted_globals.set_sid, PRRTE_CMD_LINE_TYPE_BOOL,
      "Direct the orted to separate from the current session"},

    { NULL, '\0', "tree-spawn", "tree-spawn", 0,
      &prted_globals.tree_spawn, PRRTE_CMD_LINE_TYPE_BOOL,
    ...
    { "prrte_report_bindings", '\0', "report-bindings", "report-bindings", 0,
      NULL, PRRTE_CMD_LINE_TYPE_BOOL,
...
int prrte_daemon(int argc, char *argv[])
{
    int ret = 0;
    ...
    memset(&prted_globals, 0, sizeof(prted_globals));
    ...
    cmd_line = PRRTE_NEW(prrte_cmd_line_t);
    if (PRRTE_SUCCESS != prrte_cmd_line_create(cmd_line, prrte_cmd_line_opts)) {
        PRRTE_RELEASE(cmd_line);
        exit(1);
    }
    prrte_mca_base_cmd_line_setup(cmd_line);
    if (PRRTE_SUCCESS != (ret = prrte_cmd_line_parse(cmd_line, false, false,
                                                   argc, argv))) {
        char *args = NULL;
    ...
    prrte_unsetenv(PRRTE_MCA_PREFIX"ess", &prrte_launch_environ);
    ...
    if (prted_globals.help) {
        char *args = NULL;
        args = prrte_cmd_line_get_usage_msg(cmd_line);
        prrte_show_help("help-prted.txt", "prted:usage", false,
                       argv[0], args);
        free(args);
        return 1;
    }
    ...
    if(!prrte_debug_flag &&
       ...
       prted_globals.daemonize) {
        prrte_daemon_init(NULL);
    }
        ...
        if ((int)PRRTE_PROC_MY_NAME->vpid == prted_debug_failure) {
            ...
            if (0 < prted_debug_failure_delay) {
                ...
                prrte_output(0, "%s is executing clean %s", PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME),
                            prted_globals.abort ? "abort" : "abnormal termination");
    ...
    if (prrte_debug_daemons_flag) {
        prrte_output(0, "%s orted: up and running - waiting for commands!", PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME));
    }
    ret = PRRTE_SUCCESS;
