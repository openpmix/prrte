...
typedef struct prrte_bitmap_t prrte_bitmap_t;

PRRTE_EXPORT PRRTE_CLASS_DECLARATION(prrte_bitmap_t);

/**
 * Set the maximum size of the bitmap.
 * May be reset any time, but HAS TO BE SET BEFORE prrte_bitmap_init!
 *
 * @param  bitmap     The input bitmap (IN)
 * @param  max_size   The maximum size of the bitmap in terms of bits (IN)
 * @return PRRTE error code or success
 *
 */
PRRTE_EXPORT int prrte_bitmap_set_max_size (prrte_bitmap_t *bm, int max_size);
 ...
 *
 */
PRRTE_EXPORT int prrte_bitmap_init (prrte_bitmap_t *bm, int size);
 ...
 *
 */
PRRTE_EXPORT int prrte_bitmap_set_bit(prrte_bitmap_t *bm, int bit);
 ...
 * @return PRRTE error code if the bit is out of range, else success
 *
 */
PRRTE_EXPORT int prrte_bitmap_clear_bit(prrte_bitmap_t *bm, int bit);
...
/**
  ...
  *
  */
PRRTE_EXPORT bool prrte_bitmap_is_set_bit(prrte_bitmap_t *bm, int bit);
 ...
 * @return err        PRRTE_SUCCESS on success
 */
PRRTE_EXPORT int prrte_bitmap_find_and_set_first_unset_bit(prrte_bitmap_t *bm,
                                                           int *position);
 ...
 *
 * @param bitmap The input bitmap (IN)
 * @return PRRTE error code if bm is NULL
 *
 */
PRRTE_EXPORT int prrte_bitmap_clear_all_bits(prrte_bitmap_t *bm);
 ...
 *
 */
PRRTE_EXPORT int prrte_bitmap_set_all_bits(prrte_bitmap_t *bm);
 ...
 *
 */
static inline int prrte_bitmap_size(prrte_bitmap_t *bm)
{
 ...
 * @ return PRRTE error code if something goes wrong
 */
static inline void prrte_bitmap_copy(prrte_bitmap_t *dest, prrte_bitmap_t *src)
{
 ...
 * @return PRRTE error code if the length of the two bitmaps is not equal or one is NULL.
 */
PRRTE_EXPORT int prrte_bitmap_bitwise_and_inplace(prrte_bitmap_t *dest, prrte_bitmap_t *right);
 ...
 * @return PRRTE error code if the length of the two bitmaps is not equal or one is NULL.
 */
PRRTE_EXPORT int prrte_bitmap_bitwise_or_inplace(prrte_bitmap_t *dest, prrte_bitmap_t *right);
 ...
 * @return PRRTE error code if the length of the two bitmaps is not equal or one is NULL.
 */
PRRTE_EXPORT int prrte_bitmap_bitwise_xor_inplace(prrte_bitmap_t *dest, prrte_bitmap_t *right);
 ...
 */
PRRTE_EXPORT bool prrte_bitmap_are_different(prrte_bitmap_t *left, prrte_bitmap_t *right);
 ...
 */
PRRTE_EXPORT char * prrte_bitmap_get_string(prrte_bitmap_t *bitmap);
 ...
 * @return Integer
 */
PRRTE_EXPORT int prrte_bitmap_num_unset_bits(prrte_bitmap_t *bm, int len);
 ...
 * @return Integer
 */
PRRTE_EXPORT int prrte_bitmap_num_set_bits(prrte_bitmap_t *bm, int len);

/**
 * Check a bitmap to see if any bit is set
 */
PRRTE_EXPORT bool prrte_bitmap_is_clear(prrte_bitmap_t *bm);
