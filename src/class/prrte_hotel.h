 ...
 *   given time (i.e., when an prrte timer event expires).
 ...
 *
 * There is an prrte_hotel_init() function to create a hotel, but no
...
#ifndef PRRTE_HOTEL_H
#define PRRTE_HOTEL_H
...
struct prrte_hotel_t;

/* User-supplied function to be invoked when an occupant is evicted. */
typedef void (*prrte_hotel_eviction_callback_fn_t)(struct prrte_hotel_t *hotel,
                                                  int room_num,
                                                  void *occupant);

/* Note that this is an internal data structure; it is not part of the
   public prrte_hotel interface.  Public consumers of prrte_hotel
...
typedef struct {
    void *occupant;
    prrte_event_t eviction_timer_event;
} prrte_hotel_room_t;
...
typedef struct prrte_hotel_t {
    ...
    prrte_hotel_eviction_callback_fn_t evict_callback_fn;

    /* All rooms in this hotel */
    prrte_hotel_room_t *rooms;

    /* Separate array for all the eviction callback arguments (see
       rationale above for why this is a separate array) */
    prrte_hotel_room_eviction_callback_arg_t *eviction_args;
...
PRRTE_CLASS_DECLARATION(prrte_hotel_t);
 ...
 */
PRRTE_EXPORT int prrte_hotel_init(prrte_hotel_t *hotel, int num_rooms,
                                  prrte_event_base_t *evbase,
 ...
 *
 * @return PRRTE_SUCCESS if the occupant is successfully checked in,
 * and the room parameter will contain a valid value.
 * @return PRRTE_ERR_TEMP_OUT_OF_RESOURCE is the hotel is full.  Try
 * again later.
 */
static inline int prrte_hotel_checkin(prrte_hotel_t *hotel,
                                     void *occupant,
                                     int *room_num)
{
    prrte_hotel_room_t *room;

    /* Do we have any rooms available? */
    if (PRRTE_UNLIKELY(hotel->last_unoccupied_room < 0)) {
    ...
    *room_num = hotel->unoccupied_rooms[hotel->last_unoccupied_room--];
    ...
    if (NULL != hotel->evbase) {
        prrte_event_add(&(room->eviction_timer_event),
...
/**
 * Same as prrte_hotel_checkin(), but slightly optimized for when the
 * caller *knows* that there is a room available.
 */
static inline void prrte_hotel_checkin_with_res(prrte_hotel_t *hotel,
                                     void *occupant,
                                     int *room_num)
{
    prrte_hotel_room_t *room;
 ...
 */
static inline void prrte_hotel_checkout(prrte_hotel_t *hotel, int room_num)
{
    prrte_hotel_room_t *room;
    ...
    if (PRRTE_LIKELY(NULL != room->occupant)) {
        /* Do not change this logic without also changing the same
           logic in prrte_hotel_checkout_and_return_occupant() and
           prrte_hotel.c:local_eviction_callback(). */
        room->occupant = NULL;
        if (NULL != hotel->evbase) {
            prrte_event_del(&(room->eviction_timer_event));
        }
 ...
 */
static inline void prrte_hotel_checkout_and_return_occupant(prrte_hotel_t *hotel, int room_num, void **occupant)
{
    prrte_hotel_room_t *room;
 ...
 *
 */
static inline bool prrte_hotel_is_empty (prrte_hotel_t *hotel)
{
 ...
 */
static inline void prrte_hotel_knock(prrte_hotel_t *hotel, int room_num, void **occupant)
{
    prrte_hotel_room_t *room;
    ...
    *occupant = NULL;
    ...
    room = &(hotel->rooms[room_num]);
    if (PRRTE_LIKELY(NULL != room->occupant)) {
        prrte_output (10, "occupant %p in room num %d responded to knock", room->occupant, room_num);
        *occupant = room->occupant;
    }
}
