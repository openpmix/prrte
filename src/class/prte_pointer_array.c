...
static void prte_pointer_array_construct(prte_pointer_array_t *);
static void prte_pointer_array_destruct(prte_pointer_array_t *);
static bool grow_table(prte_pointer_array_t *table, int at_least);

PRTE_CLASS_INSTANCE(prte_pointer_array_t, prte_object_t, prte_pointer_array_construct,
                    prte_pointer_array_destruct);

/*
 * prte_pointer_array constructor
 */
static void prte_pointer_array_construct(prte_pointer_array_t *array)
{
    PRTE_CONSTRUCT(&array->lock, prte_mutex_t);
    array->lowest_free = 0;
    array->number_free = 0;
    array->size = 0;
...
/*
 * prte_pointer_array destructor
 */
static void prte_pointer_array_destruct(prte_pointer_array_t *array)
{
    /* free table */
 ...
 */
static void prte_pointer_array_validate(prte_pointer_array_t *array)
{
    int i, cnt = 0;
 ...
 */
int prte_pointer_array_init(prte_pointer_array_t *array, int initial_allocation, int max_size,
                            int block_size)
{
    size_t num_bytes;
 ...
 *
 * @param table Pointer to prte_pointer_array_t object (IN)
 * @param ptr Pointer to be added to table    (IN)
 *
 * @return Array index where ptr is inserted or PRTE_ERROR if it fails
 */
int prte_pointer_array_add(prte_pointer_array_t *table, void *ptr)
{
    int index = table->size + 1;

    prte_mutex_lock(&(table->lock));

    if (table->number_free == 0) {
        /* need to grow table */
        if (!grow_table(table, index)) {
            prte_mutex_unlock(&(table->lock));
            return PRTE_ERR_OUT_OF_RESOURCE;
        }
    }
...
#if 0
    prte_pointer_array_validate(table);
#endif
    prte_mutex_unlock(&(table->lock));
    return index;
}
 ...
 */
int prte_pointer_array_set_item(prte_pointer_array_t *table, int index, void *value)
{
    assert(table != NULL);

    if (PRTE_UNLIKELY(0 > index)) {
        return PRTE_ERROR;
    }
...
#if 0
    prte_pointer_array_validate(table);
    prte_output(0,"prte_pointer_array_set_item: OUT: "
 ...
 */
bool prte_pointer_array_test_and_set_item(prte_pointer_array_t *table, int index, void *value)
{
    assert(table != NULL);
    assert(index >= 0);

#if 0
    prte_output(0,"prte_pointer_array_test_and_set_item: IN:  "
               " table %p (size %ld, lowest free %ld, number free %ld)"
               " addr[%d] = %p\n",
               table, table->size, table->lowest_free, table->number_free,
               index, table->addr[index]);
...
#if 0
    prte_pointer_array_validate(table);
    prte_output(0,"prte_pointer_array_test_and_set_item: OUT: "
               " table %p (size %ld, lowest free %ld, number free %ld)"
               " addr[%d] = %p\n",
               table, table->size, table->lowest_free, table->number_free,
               index, table->addr[index]);
...
int prte_pointer_array_set_size(prte_pointer_array_t *array, int new_size)
{
...
static bool grow_table(prte_pointer_array_t *table, int at_least)
{
    ...
    table->size = new_size;
#if 0
    prte_output(0, "grow_table %p to %d (max_size %d, block %d, number_free %d)\n",
