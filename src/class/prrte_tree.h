 ...
 * components.  A tree structure component as defined by prrte_tree_item_t
 ...
 * each item.  When a user create a type to be used for a PRRTE_CLASS_INSTANCE
 * it will contain the prrte_tree_item_t followed by any user specific
 * data.  Then the prrte_tree_item_t objects can be put in an
 * prrte_tree_t.  Hence, you create a new type that derives from
 * prrte_tree_item_t; this new type can then be used with prrte_tree_t
 * containers.
 *
 * NOTE: prrte_tree_item_t instances can only be on \em one tree at a
 * time.  Specifically, if you add an prrte_tree_item_t to one tree,
 ...
 * ensure that an prrte_tree_item_t is only one *one* tree at a time.
 ...
 * threads may [erroneously] modify an prrte_tree_item_t concurrently.
 *
 ...
 */
PRRTE_EXPORT PRRTE_CLASS_DECLARATION(prrte_tree_t);
/**
 * \internal
 *
 * Base class for items that are put in tree containers.
 */
PRRTE_EXPORT PRRTE_CLASS_DECLARATION(prrte_tree_item_t);

/**
 * \internal
 *
 * Struct of an prrte_tree_item_t
 */
typedef struct prrte_tree_item_t
{
    ...
    struct prrte_tree_item_t *prrte_tree_parent;
    /** Depth from the root item in tree */
    unsigned prrte_tree_num_ancestors;
    /* Logical rank we are compared to other siblings */
    unsigned prrte_tree_sibling_rank;
    /** Pointer to next item below same parent (or NULL if this is the
        leftmost sibling) */
    struct prrte_tree_item_t *prrte_tree_next_sibling;
    /** Pointer to previous item below same parent (or NULL if this is
        the rightmost sibling) */
    struct prrte_tree_item_t *prrte_tree_prev_sibling;

    /* Children info */
    /** Number of children */
    unsigned prrte_tree_num_children;
    /** Pointer to first child item, or NULL if there are no children */
    struct prrte_tree_item_t *prrte_tree_first_child;
    /** Pointer to last child item, or NULL if there are no children */
    struct prrte_tree_item_t *prrte_tree_last_child;

#if PRRTE_ENABLE_DEBUG
    /** Atomic reference count for debugging */
    prrte_atomic_int32_t prrte_tree_item_refcount;
    /** The tree this item belong to */
    struct prrte_tree_t* prrte_tree_item_belong_to;
#endif
} prrte_tree_item_t;
 ...
 * and initialized the pointer by the call to prrte_tree_init.
 *
 */
typedef int (*prrte_tree_comp_fn_t)(prrte_tree_item_t *item, void *key);

/**
  * The serialize function typedef. This function is called by the
  * prrte tree serialize code to serialize a tree item's user specific
  * data of a class type.
  *
  * @params item - item to serialize the user specific data from
  * @params buffer - the prrte_buffer_t to store the serialized data in.
  *
  * @returns PRRTE_SUCCESS - when successfully serialized item
  */
typedef int (*prrte_tree_item_serialize_fn_t)(prrte_tree_item_t *item,
					     prrte_buffer_t *buffer);

/**
  * The deserialize function typedef. This function is called by the
  * prrte tree deserialize code to deserialize a tree item's user
  * specific data.
  *
  * @params buffer - the prrte_buffer_t to deserialized data.
  * @params item - item to store the deserialized data into the user
  *                specific data
  *
  * @returns PRRTE_SUCCESS - when successfully deserialized item
  */
typedef int (*prrte_tree_item_deserialize_fn_t)(prrte_buffer_t *buffer,
					       prrte_tree_item_t **item);

/**
  * Get the 'key' associated with this item
  */
typedef void *(*prrte_tree_get_key_fn_t)(prrte_tree_item_t *item);

/**
 * \internal
 *
 * Struct of an prrte_tree_t
 *
 */
typedef struct prrte_tree_t
{
    ...
    volatile size_t     prrte_tree_num_items;
    /** Function to compare two items in tree */
    prrte_tree_comp_fn_t comp;
 ...
 */
static inline prrte_tree_item_t *prrte_tree_get_parent(prrte_tree_item_t *item)
{
    return ((item) ? item->prrte_tree_parent : NULL);
}
 ...
 * This function is safe to be called with a null item pointer.
 */
static inline prrte_tree_item_t *prrte_tree_get_next_sibling(prrte_tree_item_t
                                                           *item)
{
    return ((item) ? item->prrte_tree_next_sibling : NULL);
}
 ...
 * This function is safe to be called with a null item pointer.
 */
static inline prrte_tree_item_t *prrte_tree_get_prev_sibling(prrte_tree_item_t
                                                           *item)
{
    return ((item) ? item->prrte_tree_prev_sibling : NULL);
}
 ...
 *
 */
static inline prrte_tree_item_t *prrte_tree_get_first_child(prrte_tree_item_t
                                                          *item)
{
    return ((item) ? item->prrte_tree_first_child : NULL);
}
 ...
 *
 */
static inline prrte_tree_item_t *prrte_tree_get_last_child(prrte_tree_item_t
                                                         *item)
{
    return ((item) ? item->prrte_tree_last_child : NULL);
}
 ...
 *
 */
static inline bool prrte_tree_is_empty(prrte_tree_t* tree)
{
#if PRRTE_ENABLE_DEBUG
    /* Spot check that the tree is a non-null pointer */
    assert(NULL != tree);
#endif
    return (tree->prrte_tree_sentinel.prrte_tree_first_child ==
            &(tree->prrte_tree_sentinel) ? true : false);
}
 ...
 *
 */
static inline prrte_tree_item_t* prrte_tree_get_root(prrte_tree_t* tree)
{
    prrte_tree_item_t* item;
#if PRRTE_ENABLE_DEBUG
    assert(NULL != tree);
#endif
    item = tree->prrte_tree_sentinel.prrte_tree_first_child;
#if PRRTE_ENABLE_DEBUG
    /* Spot check: ensure that the first item is only on one list */
    assert(1 == item->prrte_tree_item_refcount);
    assert(tree == item->prrte_tree_item_belong_to );
#endif
    return item;
}
 ...
 * size of the list.
 */
PRRTE_EXPORT size_t prrte_tree_get_size(prrte_tree_t* tree);
 ...
 *
 */
PRRTE_EXPORT void prrte_tree_init(prrte_tree_t *tree,
                                  prrte_tree_comp_fn_t comp,
                                  prrte_tree_item_serialize_fn_t serialize,
                                  prrte_tree_item_deserialize_fn_t deserialize,
                                  prrte_tree_get_key_fn_t get_key);
 ...
 */
PRRTE_EXPORT void prrte_tree_add_child(prrte_tree_item_t *parent_item,
 ...
 */
PRRTE_EXPORT prrte_tree_item_t *prrte_tree_remove_subtree(prrte_tree_item_t *item);
 ...
 */
PRRTE_EXPORT int prrte_tree_remove_item(prrte_tree_t *tree,
                                        prrte_tree_item_t *item);
 ...
 * @param buffer The prrte buffer that contains the serialized
 *  data stream of the tree
 *
 * @returns PRRTE_SUCCESS if data has been successfully converted.
 *
 ...
 * using the prrte_dss.pack routine, that can be sent over a network.
 ...
 *
 */
PRRTE_EXPORT int prrte_tree_serialize(prrte_tree_item_t *start_item,
				      prrte_buffer_t *buffer);

/**
 * De-serialize tree data
 *
 * @param buffer The prrte buffer that is to be deserialized
 * @param start_item The item in the tree the data should be
 * deserialized into
 *
 * @returns Status of call PRRTE_SUCCESS if everything worked
 *
 * This routine takes a bytestream that was created by the
 * prrte_tree_serialize() function and deserializes it into the
 ...
 *
 */
PRRTE_EXPORT int prrte_tree_deserialize(prrte_buffer_t *buffer,
 ...
 * @returns Success/Failure
 */
PRRTE_EXPORT void * prrte_tree_get_key(prrte_tree_t *tree, prrte_tree_item_t *item);
 ...
 * @returns Success/Failure
 */
PRRTE_EXPORT int prrte_tree_dup(prrte_tree_t *from, prrte_tree_t *to);
 ...
 */
PRRTE_EXPORT int prrte_tree_copy_subtree(prrte_tree_t *from_tree, prrte_tree_item_t *from_item,
 ...
 * @returns Tree item copy
 */
PRRTE_EXPORT prrte_tree_item_t *prrte_tree_dup_item(prrte_tree_t *base, prrte_tree_item_t *from);
 ...
 */
PRRTE_EXPORT int prrte_tree_num_children(prrte_tree_item_t *parent);
 ...
 */
PRRTE_EXPORT int prrte_tree_compare(prrte_tree_t *left, prrte_tree_t *right);
 ...
 *
 */
PRRTE_EXPORT prrte_tree_item_t *prrte_tree_find_with(prrte_tree_item_t *item,
                                                    void *key);
END_C_DECLS
