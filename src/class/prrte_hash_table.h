...
#ifndef PRRTE_HASH_TABLE_H
#define PRRTE_HASH_TABLE_H
...
PRRTE_EXPORT PRRTE_CLASS_DECLARATION(prrte_hash_table_t);

struct prrte_hash_table_t
{
    prrte_object_t        super;          /**< subclass of prrte_object_t */
    struct prrte_hash_element_t * ht_table;       /**< table of elements (opaque to users) */
    ...
    const struct prrte_hash_type_methods_t * ht_type_methods;
};
typedef struct prrte_hash_table_t prrte_hash_table_t;
...
PRRTE_EXPORT int prrte_hash_table_init(prrte_hash_table_t* ht, size_t table_size);

/* this could be the new init if people wanted a more general API */
PRRTE_EXPORT int prrte_hash_table_init2(prrte_hash_table_t* ht, size_t estimated_max_size,
...
static inline size_t prrte_hash_table_get_size(prrte_hash_table_t *ht)
{
    return ht->ht_size;
}
 ...
 *
 *  @param   table   The input hash table (IN).
 *  @return  PRRTE return code.
 *
 */

PRRTE_EXPORT int prrte_hash_table_remove_all(prrte_hash_table_t *ht);
 ...
 *           - PRRTE_SUCCESS       if key was found
 *           - PRRTE_ERR_NOT_FOUND if key was not found
 *           - PRRTE_ERROR         other error
 *
 */

PRRTE_EXPORT int prrte_hash_table_get_value_uint32(prrte_hash_table_t* table, uint32_t key,
                                                   void** ptr);
...
PRRTE_EXPORT int prrte_hash_table_set_value_uint32(prrte_hash_table_t* table, uint32_t key, void* value);
...
PRRTE_EXPORT int prrte_hash_table_remove_value_uint32(prrte_hash_table_t* table, uint32_t key);
...
PRRTE_EXPORT int prrte_hash_table_get_value_uint64(prrte_hash_table_t *table, uint64_t key,
                                                   void **ptr);
...
PRRTE_EXPORT int prrte_hash_table_set_value_uint64(prrte_hash_table_t *table, uint64_t key, void* value);
...
PRRTE_EXPORT int prrte_hash_table_remove_value_uint64(prrte_hash_table_t *table, uint64_t key);
...
PRRTE_EXPORT int prrte_hash_table_get_value_ptr(prrte_hash_table_t *table, const void* key,
...
PRRTE_EXPORT int prrte_hash_table_set_value_ptr(prrte_hash_table_t *table, const void* key, size_t keylen, void* value);
...
PRRTE_EXPORT int prrte_hash_table_remove_value_ptr(prrte_hash_table_t *table, const void* key, size_t keylen);
...
PRRTE_EXPORT int prrte_hash_table_get_first_key_uint32(prrte_hash_table_t *table, uint32_t *key,
...
PRRTE_EXPORT int prrte_hash_table_get_next_key_uint32(prrte_hash_table_t *table, uint32_t *key,
...
PRRTE_EXPORT int prrte_hash_table_get_first_key_uint64(prrte_hash_table_t *table, uint64_t *key,
                                       void **value, void **node);
...
PRRTE_EXPORT int prrte_hash_table_get_next_key_uint64(prrte_hash_table_t *table, uint64_t *key,
                                       void **value, void *in_node,
                                       void **out_node);
...
PRRTE_EXPORT int prrte_hash_table_get_first_key_ptr(prrte_hash_table_t *table, void* *key,
...
PRRTE_EXPORT int prrte_hash_table_get_next_key_ptr(prrte_hash_table_t *table, void* *key,
                                       size_t *key_size, void **value,
                                       void *in_node, void **out_node);



PRRTE_EXPORT PRRTE_CLASS_DECLARATION(prrte_proc_table_t);

struct prrte_proc_table_t
{
    prrte_hash_table_t    super;          /**< subclass of prrte_object_t */
...
typedef struct prrte_proc_table_t prrte_proc_table_t;
...
PRRTE_EXPORT int prrte_proc_table_init(prrte_proc_table_t* pt, size_t jobids, size_t vpids);
...
PRRTE_EXPORT int prrte_proc_table_remove_all(prrte_proc_table_t *pt);

/**
 *  Retrieve value via prrte_process_name_t key.
 *
 *  @param   pt      The input hash table (IN).
 *  @param   key     The input key (IN).
 *  @param   ptr     The value associated with the key
...
PRRTE_EXPORT int prrte_proc_table_get_value(prrte_proc_table_t* pt, prrte_process_name_t key,
                                                   void** ptr);

/**
 *  Set value based on prrte_process_name_t key.
 *
 *  @param   pt      The input hash table (IN).
 *  @param   key     The input key (IN).
 *  @param   value   The value to be associated with the key (IN).
...
PRRTE_EXPORT int prrte_proc_table_set_value(prrte_proc_table_t* pt, prrte_process_name_t key, void* value);

/**
 *  Remove value based on prrte_process_name_t key.
 *
 *  @param   pt      The input hash table (IN).
 *  @param   key     The input key (IN).
 *  @return  PRRTE return code.
...
PRRTE_EXPORT int prrte_proc_table_remove_value(prrte_proc_table_t* pt, prrte_process_name_t key);


/**
 *  Get the first prrte_process_name_t key from the hash table, which can be used later to
 *  get the next key
...
PRRTE_EXPORT int prrte_proc_table_get_first_key(prrte_proc_table_t *pt, prrte_process_name_t *key,
                                                void **value, void **node1, void **node2);


/**
 *  Get the next prrte_process_name_t key from the hash table, knowing the current key
 *  @param  pt       The hash table pointer (IN)
 *  @param  key      The key (OUT)
 *  @param  value    The value corresponding to this key (OUT)
...
PRRTE_EXPORT int prrte_proc_table_get_next_key(prrte_proc_table_t *pt, prrte_process_name_t *key,
 ...
 *
 * This macro provides a simple way to loop over the items in an prrte_hash_table_t. It
 * is not safe to call prrte_hash_table_remove* from within the loop.
 *
 * Example Usage:
 *
 * uint64_t key;
 * void * value;
 * PRRTE_HASH_TABLE_FOREACH(key, uint64, value, ht) {
 *    do_something(key, value);
 * }
 */
#define PRRTE_HASH_TABLE_FOREACH(key, type, value, ht) \
  for (void *_nptr=NULL;                                   \
       PRRTE_SUCCESS == prrte_hash_table_get_next_key_##type(ht, &key, (void **)&value, _nptr, &_nptr);)

END_C_DECLS

#endif  /* PRRTE_HASH_TABLE_H */
