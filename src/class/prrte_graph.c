...
static void prrte_graph_vertex_construct(prrte_graph_vertex_t *vertex);
static void prrte_graph_vertex_destruct(prrte_graph_vertex_t *vertex);

PRRTE_CLASS_INSTANCE(
    prrte_graph_vertex_t,
    prrte_list_item_t,
    prrte_graph_vertex_construct,
    prrte_graph_vertex_destruct
);


static void prrte_graph_edge_construct(prrte_graph_edge_t *edge);
static void prrte_graph_edge_destruct(prrte_graph_edge_t *edge);

PRRTE_CLASS_INSTANCE(
    prrte_graph_edge_t,
    prrte_list_item_t,
    prrte_graph_edge_construct,
    prrte_graph_edge_destruct
);

static void prrte_graph_construct(prrte_graph_t *graph);
static void prrte_graph_destruct(prrte_graph_t *graph);

PRRTE_CLASS_INSTANCE(
    prrte_graph_t,
    prrte_object_t,
    prrte_graph_construct,
    prrte_graph_destruct
);

static void prrte_adjacency_list_construct(prrte_adjacency_list_t *aj_list);
static void prrte_adjacency_list_destruct(prrte_adjacency_list_t *aj_list);

PRRTE_CLASS_INSTANCE(
    prrte_adjacency_list_t,
    prrte_list_item_t,
    prrte_adjacency_list_construct,
    prrte_adjacency_list_destruct
);
...
/*
 *
 *      prrte_graph_vertex_t interface
 *
 */
...
static void prrte_graph_construct(prrte_graph_t *graph)
{
    graph->adjacency_list = PRRTE_NEW(prrte_list_t);
...
static void prrte_graph_destruct(prrte_graph_t *graph)
{
    PRRTE_LIST_RELEASE(graph->adjacency_list);
    graph->number_of_vertices = 0;
    graph->number_of_edges = 0;
}
...
/*
 *
 *     prrte_adjacency_list  interface
 *
 */
...
{
    aj_list->vertex = NULL;
    aj_list->edges = PRRTE_NEW(prrte_list_t);
}

static void prrte_adjacency_list_destruct(prrte_adjacency_list_t *aj_list)
{
   aj_list->vertex = NULL;
   PRRTE_LIST_RELEASE(aj_list->edges);
}
 ...
 * @param vertex
 */
static void delete_all_edges_conceded_to_vertex(prrte_graph_t *graph, prrte_graph_vertex_t *vertex)
{
    prrte_adjacency_list_t *aj_list;
    prrte_graph_edge_t *edge, *next;

    /**
     * for all the adjacency list in the graph
     */
    PRRTE_LIST_FOREACH(aj_list, graph->adjacency_list, prrte_adjacency_list_t) {
        /**
         * for all the edges in the adjacency list
         */
        PRRTE_LIST_FOREACH_SAFE(edge, next, aj_list->edges, prrte_graph_edge_t) {
            /**
             * if the edge is ended in the vertex
             */
            if (edge->end == vertex) {
                /* Delete this edge  */
                prrte_list_remove_item(edge->in_adj_list->edges, (prrte_list_item_t*)edge);
                /* distract this edge */
                PRRTE_RELEASE(edge);
            }
        }
 ...
 */
void prrte_graph_add_vertex(prrte_graph_t *graph, prrte_graph_vertex_t *vertex)
{
   prrte_adjacency_list_t *aj_list;
   ...
   aj_list = PRRTE_NEW(prrte_adjacency_list_t);
   ...
   prrte_list_append(graph->adjacency_list, (prrte_list_item_t*)aj_list);
 ...
 */
int prrte_graph_add_edge(prrte_graph_t *graph, prrte_graph_edge_t *edge)
{
    prrte_adjacency_list_t *aj_list, *start_aj_list= NULL;
    ...
    prrte_list_append(start_aj_list->edges, (prrte_list_item_t*)edge);
 ...
 */
void prrte_graph_remove_edge (prrte_graph_t *graph, prrte_graph_edge_t *edge)
{
...
void prrte_graph_remove_vertex(prrte_graph_t *graph, prrte_graph_vertex_t *vertex)
{
    prrte_adjacency_list_t *adj_list;
     ...
     * destruct it.
     */
    prrte_list_remove_item(graph->adjacency_list, (prrte_list_item_t*)adj_list);
    PRRTE_RELEASE(adj_list);
    /**
     * delete all the edges that connected *to* the vertex.
     */
    delete_all_edges_conceded_to_vertex(graph, vertex);
    /* destruct the vertex */
    PRRTE_RELEASE(vertex);
 ...
 *         connected.
 */
uint32_t prrte_graph_adjacent(prrte_graph_t *graph, prrte_graph_vertex_t *vertex1, prrte_graph_vertex_t *vertex2)
{
    prrte_adjacency_list_t *adj_list;
    prrte_graph_edge_t *edge;

    /**
     * Verify that the first vertex belongs to the graph.
     */
    if (graph != vertex1->in_graph) {
        PRRTE_OUTPUT((0,"prrte_graph_adjacent 1 Vertex1 %p not in the graph %p\n",(void *)vertex1,(void *)graph));
    ...
    if (graph != vertex2->in_graph) {
        PRRTE_OUTPUT((0,"prrte_graph_adjacent 2 Vertex2 %p not in the graph %p\n",(void *)vertex2,(void *)graph));
        return DISTANCE_INFINITY;
    }
    /**
     ...
     * vertex.
     */
    adj_list = (prrte_adjacency_list_t *) vertex1->in_adj_list;
    PRRTE_LIST_FOREACH(edge, adj_list->edges, prrte_graph_edge_t) {
        if (edge->end == vertex2) {
 ...
 * @return int
 */
int prrte_graph_get_order(prrte_graph_t *graph)
{
 ...
 * @return int
 */
int prrte_graph_get_size(prrte_graph_t *graph)
{
 ...
 */
prrte_graph_vertex_t *prrte_graph_find_vertex(prrte_graph_t *graph, void *vertex_data)
{
    prrte_adjacency_list_t *aj_list;
 ...
 */
int prrte_graph_get_graph_vertices(prrte_graph_t *graph, prrte_pointer_array_t *vertices_list)
{
    prrte_adjacency_list_t *aj_list;
    ...
    PRRTE_LIST_FOREACH(aj_list, graph->adjacency_list, prrte_adjacency_list_t) {
        ...
        prrte_pointer_array_add(vertices_list,(void *)aj_list->vertex);
    }
 ...
 */
int prrte_graph_get_adjacent_vertices(prrte_graph_t *graph, prrte_graph_vertex_t *vertex, prrte_value_array_t *adjacents)
{
    prrte_adjacency_list_t *adj_list;
    prrte_graph_edge_t *edge;
    ...
    if (graph != vertex->in_graph) {
        PRRTE_OUTPUT((0,"Vertex %p not in the graph %p\n", (void *)vertex, (void *)graph));
        return 0;
    }
    /**
     * find the adjacency list that this vertex belongs to
     */
    adj_list = (prrte_adjacency_list_t *) vertex->in_adj_list;
    /* find the number of adjcents of this vertex */
    adjacents_number = prrte_list_get_size(adj_list->edges);
    ...
    PRRTE_LIST_FOREACH(edge, adj_list->edges, prrte_graph_edge_t) {
        ...
        prrte_value_array_append_item(adjacents, &distance_from);
    }
...
uint32_t prrte_graph_spf(prrte_graph_t *graph, prrte_graph_vertex_t *vertex1, prrte_graph_vertex_t *vertex2)
{
    prrte_value_array_t *distance_array;
     ...
     */
    if (graph != vertex1->in_graph) {
        PRRTE_OUTPUT((0,"prrte_graph_spf 1 Vertex1 %p not in the graph %p\n",(void *)vertex1,(void *)graph));
        return DISTANCE_INFINITY;
    }
    /**
     * Verify that the second vertex belongs to the graph.
     */
    if (graph != vertex2->in_graph) {
        PRRTE_OUTPUT((0,"prrte_graph_spf 2 Vertex2 %p not in the graph %p\n",(void *)vertex2,(void *)graph));
        return DISTANCE_INFINITY;
    }
    /**
     * Run Dijkstra algorithm on the graph from the start vertex.
     */
    distance_array = PRRTE_NEW(prrte_value_array_t);
    prrte_value_array_init(distance_array, sizeof(vertex_distance_from_t));
    prrte_value_array_reserve(distance_array,50);
    items_in_distance_array = prrte_graph_dijkstra(graph, vertex1, distance_array);
    /**
     * find the end vertex in the distance array that Dijkstra
     * algorithm returned.
     */
    for (i = 0; i < items_in_distance_array; i++) {
        vertex_distance = prrte_value_array_get_item(distance_array, i);
        if (vertex_distance->vertex == vertex2) {
            spf = vertex_distance->weight;
            break;
        }
    }
    PRRTE_RELEASE(distance_array);
 ...
 */
uint32_t prrte_graph_dijkstra(prrte_graph_t *graph, prrte_graph_vertex_t *vertex, prrte_value_array_t *distance_array)
{
    int graph_order;
    ...
    while (number_of_items_in_q > 0) {
        ...
        for (i = 0; i < number_of_items_in_q; i++) {
            weight = prrte_graph_adjacent(graph, current_vertex->vertex, q_start[i].vertex);
            /**
    ...
    for (i = 0; i < graph_order-1; i++) {
        prrte_value_array_append_item(distance_array, (void *)&(Q[i+1]));
    }
 ...
 */
void prrte_graph_duplicate(prrte_graph_t **dest, prrte_graph_t *src)
{
    prrte_adjacency_list_t *aj_list;
    prrte_graph_vertex_t *vertex;
    prrte_graph_edge_t *edge, *new_edge;

    /* construct a new graph */
    *dest = PRRTE_NEW(prrte_graph_t);
    /* Run on all the vertices of the src graph */
    PRRTE_LIST_FOREACH(aj_list, src->adjacency_list, prrte_adjacency_list_t) {
        /* for each vertex in the src graph, construct a new vertex */
        vertex = PRRTE_NEW(prrte_graph_vertex_t);
        ...
        prrte_graph_add_vertex(*dest, vertex);
    }
    /**
    ...
    PRRTE_LIST_FOREACH(aj_list, src->adjacency_list, prrte_adjacency_list_t) {
        ...
        PRRTE_LIST_FOREACH(edge, aj_list->edges, prrte_graph_edge_t) {
            /* construct new edge for the new graph */
            new_edge = PRRTE_NEW(prrte_graph_edge_t);
            ...
            prrte_graph_add_edge(*dest, new_edge);
        }
    }
}
 ...
 * @param graph
 */
void prrte_graph_print(prrte_graph_t *graph)
{
    prrte_adjacency_list_t *aj_list;
    prrte_graph_edge_t *edge;
    ...
    PRRTE_LIST_FOREACH(aj_list, graph->adjacency_list, prrte_adjacency_list_t) {
        ...
        /* print vertex */
        prrte_output(0, "V(%s) Connections:",tmp_str1);
        ...
        PRRTE_LIST_FOREACH(edge, aj_list->edges, prrte_graph_edge_t) {
            ...
            /* print the edge */
            prrte_output(0, "    E(%s -> %d -> %s)",tmp_str1, edge->weight, tmp_str2);
            if (need_free2) {
                free(tmp_str2);
            }
        }
