 ...
 * in the prrte_bitmap_t struct
 */
#define SIZE_OF_BASE_TYPE  64

static void prrte_bitmap_construct(prrte_bitmap_t *bm);
static void prrte_bitmap_destruct(prrte_bitmap_t *bm);

PRRTE_CLASS_INSTANCE(prrte_bitmap_t, prrte_object_t,
                   prrte_bitmap_construct, prrte_bitmap_destruct);


static void
prrte_bitmap_construct(prrte_bitmap_t *bm)
{
    bm->bitmap = NULL;
    bm->array_size = 0;
    bm->max_size = INT_MAX;
...
static void
prrte_bitmap_destruct(prrte_bitmap_t *bm)
{
    if (NULL != bm->bitmap) {
        free(bm->bitmap);
        bm->bitmap = NULL;
...
int prrte_bitmap_set_max_size (prrte_bitmap_t *bm, int max_size)
{
    if (NULL == bm) {
...
int
prrte_bitmap_init(prrte_bitmap_t *bm, int size)
{
    /*
    ...
    prrte_bitmap_clear_all_bits(bm);
    return PRRTE_SUCCESS;
}


int
prrte_bitmap_set_bit(prrte_bitmap_t *bm, int bit)
{
...
int
prrte_bitmap_clear_bit(prrte_bitmap_t *bm, int bit)
{
    int index, offset;
...
bool
prrte_bitmap_is_set_bit(prrte_bitmap_t *bm, int bit)
{
    int index, offset;
...
int
prrte_bitmap_clear_all_bits(prrte_bitmap_t *bm)
{
    if (NULL == bm) {
        return PRRTE_ERR_BAD_PARAM;
    }
...
int
prrte_bitmap_set_all_bits(prrte_bitmap_t *bm)
{
    if (NULL == bm) {
        return PRRTE_ERR_BAD_PARAM;
    }
...
int
prrte_bitmap_find_and_set_first_unset_bit(prrte_bitmap_t *bm, int *position)
{
    int i = 0;
    ...
    *position = 0;
    ...
    if (i == bm->array_size) {
        ...
        *position = bm->array_size * SIZE_OF_BASE_TYPE;
        return prrte_bitmap_set_bit(bm, *position);
    }
...
int prrte_bitmap_bitwise_and_inplace(prrte_bitmap_t *dest, prrte_bitmap_t *right)
{
    int i;
...
int prrte_bitmap_bitwise_or_inplace(prrte_bitmap_t *dest, prrte_bitmap_t *right)
{
    int i;
...
int prrte_bitmap_bitwise_xor_inplace(prrte_bitmap_t *dest, prrte_bitmap_t *right)
{
    int i;
...
bool prrte_bitmap_are_different(prrte_bitmap_t *left, prrte_bitmap_t *right)
{
    int i;
    ...
    if( prrte_bitmap_size(left) != prrte_bitmap_size(right) ) {
        return true;
    }
...
char * prrte_bitmap_get_string(prrte_bitmap_t *bitmap)
{
    int i;
    ...
    for( i = 0; i < (bitmap->array_size * SIZE_OF_BASE_TYPE); ++i) {
        if( prrte_bitmap_is_set_bit(bitmap, i) ) {
            bitmap_str[i] = 'X';
        } else {
            bitmap_str[i] = '_';
        }
...
int prrte_bitmap_num_unset_bits(prrte_bitmap_t *bm, int len)
{
    return (len - prrte_bitmap_num_set_bits(bm, len));
}

int prrte_bitmap_num_set_bits(prrte_bitmap_t *bm, int len)
{
    int i, cnt = 0;
    uint64_t val;
...
bool prrte_bitmap_is_clear(prrte_bitmap_t *bm)
{
    int i;
