...
static void prrte_pointer_array_construct(prrte_pointer_array_t *);
static void prrte_pointer_array_destruct(prrte_pointer_array_t *);
static bool grow_table(prrte_pointer_array_t *table, int at_least);

PRRTE_CLASS_INSTANCE(prrte_pointer_array_t, prrte_object_t,
                   prrte_pointer_array_construct,
                   prrte_pointer_array_destruct);

/*
 * prrte_pointer_array constructor
 */
static void prrte_pointer_array_construct(prrte_pointer_array_t *array)
{
    PRRTE_CONSTRUCT(&array->lock, prrte_mutex_t);
    array->lowest_free = 0;
    array->number_free = 0;
    array->size = 0;
...
/*
 * prrte_pointer_array destructor
 */
static void prrte_pointer_array_destruct(prrte_pointer_array_t *array)
{
    /* free table */
 ...
 */
static void prrte_pointer_array_validate(prrte_pointer_array_t *array)
{
    int i, cnt = 0;
 ...
 */
int prrte_pointer_array_init(prrte_pointer_array_t* array,
 ...
 *
 * @param table Pointer to prrte_pointer_array_t object (IN)
 * @param ptr Pointer to be added to table    (IN)
 *
 * @return Array index where ptr is inserted or PRRTE_ERROR if it fails
 */
int prrte_pointer_array_add(prrte_pointer_array_t *table, void *ptr)
{
...
#if 0
    prrte_pointer_array_validate(table);
#endif
    prrte_mutex_unlock(&(table->lock));
    return index;
}
 ...
 */
int prrte_pointer_array_set_item(prrte_pointer_array_t *table, int index,
                                void * value)
{
    assert(table != NULL);

    if (PRRTE_UNLIKELY(0 > index)) {
        return PRRTE_ERROR;
    }
...
#if 0
    prrte_pointer_array_validate(table);
    prrte_output(0,"prrte_pointer_array_set_item: OUT: "
 ...
 */
bool prrte_pointer_array_test_and_set_item (prrte_pointer_array_t *table,
...
#if 0
    prrte_output(0,"prrte_pointer_array_test_and_set_item: IN:  "
               " table %p (size %ld, lowest free %ld, number free %ld)"
               " addr[%d] = %p\n",
               table, table->size, table->lowest_free, table->number_free,
               index, table->addr[index]);
...
#if 0
    prrte_pointer_array_validate(table);
    prrte_output(0,"prrte_pointer_array_test_and_set_item: OUT: "
               " table %p (size %ld, lowest free %ld, number free %ld)"
               " addr[%d] = %p\n",
               table, table->size, table->lowest_free, table->number_free,
               index, table->addr[index]);
...
int prrte_pointer_array_set_size(prrte_pointer_array_t *array, int new_size)
{
...
static bool grow_table(prrte_pointer_array_t *table, int at_least)
{
    ...
    table->size = new_size;
#if 0
    prrte_output(0, "grow_table %p to %d (max_size %d, block %d, number_free %d)\n",
