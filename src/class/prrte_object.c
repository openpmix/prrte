 ...
 * @file
 *
 * Implementation of prrte_object_t, the base prrte foundation class
 */
 ...
 */
prrte_class_t prrte_object_t_class = {
    "prrte_object_t",      /* name */
    NULL,                 /* parent class */
    NULL,                 /* constructor */
    NULL,                 /* destructor */
    ...
    sizeof(prrte_object_t) /* size of the prrte object */
};

int prrte_class_init_epoch = 1;

/*
 * Local variables
 */
static prrte_atomic_lock_t class_lock = PRRTE_ATOMIC_LOCK_INIT;
 ...
 * Local functions
 */
static void save_class(prrte_class_t *cls);
 ...
 */
void prrte_class_initialize(prrte_class_t *cls)
{
    prrte_class_t *c;
    ...
    if (prrte_class_init_epoch == cls->cls_initialized) {
        prrte_atomic_unlock(&class_lock);
        return;
    }
    ...
    cls->cls_construct_array =
        (void (**)(prrte_object_t*))malloc((cls_construct_array_count +
                                           cls_destruct_array_count + 2) *
                                          sizeof(prrte_construct_t) );
    ...
    *cls_destruct_array = NULL;  /* end marker for the destructors */
    ...
    cls->cls_initialized = prrte_class_init_epoch;
    save_class(cls);
...
int prrte_class_finalize(void)
{
    ...
    if (INT_MAX == prrte_class_init_epoch) {
        prrte_class_init_epoch = 1;
    } else {
        prrte_class_init_epoch++;
    }
...
static void expand_array(void)
{
    ...
    classes = (void**)realloc(classes, sizeof(prrte_class_t*) * max_classes);
    if (NULL == classes) {
