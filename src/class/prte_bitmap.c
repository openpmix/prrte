 ...
 * in the prte_bitmap_t struct
 */
#define SIZE_OF_BASE_TYPE  64

static void prte_bitmap_construct(prte_bitmap_t *bm);
static void prte_bitmap_destruct(prte_bitmap_t *bm);

PRTE_CLASS_INSTANCE(prte_bitmap_t, prte_object_t,
                   prte_bitmap_construct, prte_bitmap_destruct);


static void
prte_bitmap_construct(prte_bitmap_t *bm)
{
    bm->bitmap = NULL;
    bm->array_size = 0;
    bm->max_size = INT_MAX;
...
static void
prte_bitmap_destruct(prte_bitmap_t *bm)
{
    if (NULL != bm->bitmap) {
        free(bm->bitmap);
        bm->bitmap = NULL;
...
int prte_bitmap_set_max_size (prte_bitmap_t *bm, int max_size)
{
    if (NULL == bm) {
...
int
prte_bitmap_init(prte_bitmap_t *bm, int size)
{
    /*
...
int
prte_bitmap_set_bit(prte_bitmap_t *bm, int bit)
{
...
int
prte_bitmap_clear_bit(prte_bitmap_t *bm, int bit)
{
    int index, offset;
...
bool
prte_bitmap_is_set_bit(prte_bitmap_t *bm, int bit)
{
    int index, offset;
...
int
prte_bitmap_clear_all_bits(prte_bitmap_t *bm)
{
    if (NULL == bm) {
        return PRTE_ERR_BAD_PARAM;
    }
...
int
prte_bitmap_set_all_bits(prte_bitmap_t *bm)
{
    if (NULL == bm) {
        return PRTE_ERR_BAD_PARAM;
    }
...
int
prte_bitmap_find_and_set_first_unset_bit(prte_bitmap_t *bm, int *position)
{
    int i = 0;
    ...
    *position = 0;
    ...
    if (i == bm->array_size) {
        ...
        *position = bm->array_size * SIZE_OF_BASE_TYPE;
        return prte_bitmap_set_bit(bm, *position);
    }
...
int prte_bitmap_bitwise_and_inplace(prte_bitmap_t *dest, prte_bitmap_t *right)
{
    int i;
...
int prte_bitmap_bitwise_or_inplace(prte_bitmap_t *dest, prte_bitmap_t *right)
{
    int i;
...
int prte_bitmap_bitwise_xor_inplace(prte_bitmap_t *dest, prte_bitmap_t *right)
{
    int i;
...
bool prte_bitmap_are_different(prte_bitmap_t *left, prte_bitmap_t *right)
{
    int i;
    ...
    if( prte_bitmap_size(left) != prte_bitmap_size(right) ) {
        return true;
    }
...
char * prte_bitmap_get_string(prte_bitmap_t *bitmap)
{
    int i;
    ...
    for( i = 0; i < (bitmap->array_size * SIZE_OF_BASE_TYPE); ++i) {
        if( prte_bitmap_is_set_bit(bitmap, i) ) {
            bitmap_str[i] = 'X';
        } else {
            bitmap_str[i] = '_';
        }
...
int prte_bitmap_num_unset_bits(prte_bitmap_t *bm, int len)
{
    return (len - prte_bitmap_num_set_bits(bm, len));
}

int prte_bitmap_num_set_bits(prte_bitmap_t *bm, int len)
{
    int i, cnt = 0;
    uint64_t val;
...
bool prte_bitmap_is_clear(prte_bitmap_t *bm)
{
    int i;
