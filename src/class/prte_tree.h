 ...
 * @file
 *
 * The prte_tree_t interface is used to provide a generic
 * tree list container for PRTE.  It was inspired by the prte_list_t
 ...
 * components.  A tree structure component as defined by prte_tree_item_t
 ...
 * each item.  When a user create a type to be used for a PRTE_CLASS_INSTANCE
 * it will contain the prte_tree_item_t followed by any user specific
 * data.  Then the prte_tree_item_t objects can be put in an
 * prte_tree_t.  Hence, you create a new type that derives from
 * prte_tree_item_t; this new type can then be used with prte_tree_t
 * containers.
 *
 * NOTE: prte_tree_item_t instances can only be on \em one tree at a
 * time.  Specifically, if you add an prte_tree_item_t to one tree,
 ...
 * ensure that an prte_tree_item_t is only one *one* tree at a time.
 ...
 * threads may [erroneously] modify an prte_tree_item_t concurrently.
 *
 ...
 */
PRTE_EXPORT PRTE_CLASS_DECLARATION(prte_tree_t);
/**
 * \internal
 *
 * Base class for items that are put in tree containers.
 */
PRTE_EXPORT PRTE_CLASS_DECLARATION(prte_tree_item_t);

/**
 * \internal
 *
 * Struct of an prte_tree_item_t
 */
typedef struct prte_tree_item_t
{
    /** Generic parent class for all PRTE objects */
    prte_object_t super;
    /** Pointer to the tree this item belongs to */
    struct prte_tree_t *prte_tree_container;
    /* Parent info */
    /** Pointer to parent tree item */
    struct prte_tree_item_t *prte_tree_parent;
    /** Depth from the root item in tree */
    unsigned prte_tree_num_ancestors;
    /* Logical rank we are compared to other siblings */
    unsigned prte_tree_sibling_rank;
    /** Pointer to next item below same parent (or NULL if this is the
        leftmost sibling) */
    struct prte_tree_item_t *prte_tree_next_sibling;
    /** Pointer to previous item below same parent (or NULL if this is
        the rightmost sibling) */
    struct prte_tree_item_t *prte_tree_prev_sibling;

    /* Children info */
    /** Number of children */
    unsigned prte_tree_num_children;
    /** Pointer to first child item, or NULL if there are no children */
    struct prte_tree_item_t *prte_tree_first_child;
    /** Pointer to last child item, or NULL if there are no children */
    struct prte_tree_item_t *prte_tree_last_child;

#if PRTE_ENABLE_DEBUG
    /** Atomic reference count for debugging */
    prte_atomic_int32_t prte_tree_item_refcount;
    /** The tree this item belong to */
    struct prte_tree_t* prte_tree_item_belong_to;
#endif
} prte_tree_item_t;
 ...
 * and initialized the pointer by the call to prte_tree_init.
 *
 */
typedef int (*prte_tree_comp_fn_t)(prte_tree_item_t *item, void *key);

/**
  * The serialize function typedef. This function is called by the
  * prte tree serialize code to serialize a tree item's user specific
  * data of a class type.
  *
  * @params item - item to serialize the user specific data from
  * @params buffer - the prte_buffer_t to store the serialized data in.
  *
  * @returns PRTE_SUCCESS - when successfully serialized item
  */
typedef int (*prte_tree_item_serialize_fn_t)(prte_tree_item_t *item,
					     prte_buffer_t *buffer);

/**
  * The deserialize function typedef. This function is called by the
  * prte tree deserialize code to deserialize a tree item's user
  * specific data.
  *
  * @params buffer - the prte_buffer_t to deserialized data.
  * @params item - item to store the deserialized data into the user
  *                specific data
  *
  * @returns PRTE_SUCCESS - when successfully deserialized item
  */
typedef int (*prte_tree_item_deserialize_fn_t)(prte_buffer_t *buffer,
					       prte_tree_item_t **item);

/**
  * Get the 'key' associated with this item
  */
typedef void *(*prte_tree_get_key_fn_t)(prte_tree_item_t *item);

/**
 * \internal
 *
 * Struct of an prte_tree_t
 *
 */
typedef struct prte_tree_t
{
    /** Generic parent class for all PRTE objects */
    prte_object_t       super;
    /** Guard item of the tree that child points to root */
    prte_tree_item_t    prte_tree_sentinel;
    /** Quick reference to the number of items in the tree */
    volatile size_t     prte_tree_num_items;
    /** Function to compare two items in tree */
    prte_tree_comp_fn_t comp;
 ...
 */
static inline prte_tree_item_t *prte_tree_get_parent(prte_tree_item_t *item)
{
    return ((item) ? item->prte_tree_parent : NULL);
}
 ...
 * This function is safe to be called with a null item pointer.
 */
static inline prte_tree_item_t *prte_tree_get_next_sibling(prte_tree_item_t
                                                           *item)
{
    return ((item) ? item->prte_tree_next_sibling : NULL);
}
 ...
 * This function is safe to be called with a null item pointer.
 */
static inline prte_tree_item_t *prte_tree_get_prev_sibling(prte_tree_item_t
                                                           *item)
{
    return ((item) ? item->prte_tree_prev_sibling : NULL);
}
 ...
 *
 */
static inline prte_tree_item_t *prte_tree_get_first_child(prte_tree_item_t
                                                          *item)
{
    return ((item) ? item->prte_tree_first_child : NULL);
}
 ...
 *
 */
static inline prte_tree_item_t *prte_tree_get_last_child(prte_tree_item_t
                                                         *item)
{
    return ((item) ? item->prte_tree_last_child : NULL);
}
 ...
 *
 */
static inline bool prte_tree_is_empty(prte_tree_t* tree)
{
#if PRTE_ENABLE_DEBUG
    /* Spot check that the tree is a non-null pointer */
    assert(NULL != tree);
#endif
    return (tree->prte_tree_sentinel.prte_tree_first_child ==
            &(tree->prte_tree_sentinel) ? true : false);
}
 ...
 *
 */
static inline prte_tree_item_t* prte_tree_get_root(prte_tree_t* tree)
{
    prte_tree_item_t* item;
#if PRTE_ENABLE_DEBUG
    assert(NULL != tree);
#endif
    item = tree->prte_tree_sentinel.prte_tree_first_child;
#if PRTE_ENABLE_DEBUG
    /* Spot check: ensure that the first item is only on one list */
    assert(1 == item->prte_tree_item_refcount);
    assert(tree == item->prte_tree_item_belong_to );
#endif
    return item;
}
 ...
 * size of the list.
 */
PRTE_EXPORT size_t prte_tree_get_size(prte_tree_t* tree);
 ...
 *
 */
PRTE_EXPORT void prte_tree_init(prte_tree_t *tree,
                                  prte_tree_comp_fn_t comp,
                                  prte_tree_item_serialize_fn_t serialize,
                                  prte_tree_item_deserialize_fn_t deserialize,
                                  prte_tree_get_key_fn_t get_key);
 ...
 */
PRTE_EXPORT void prte_tree_add_child(prte_tree_item_t *parent_item,
 ...
 */
PRTE_EXPORT prte_tree_item_t *prte_tree_remove_subtree(prte_tree_item_t *item);
 ...
 */
PRTE_EXPORT int prte_tree_remove_item(prte_tree_t *tree,
                                        prte_tree_item_t *item);
 ...
 * @param buffer The prte buffer that contains the serialized
 *  data stream of the tree
 *
 * @returns PRTE_SUCCESS if data has been successfully converted.
 *
 ...
 * using the prte_dss.pack routine, that can be sent over a network.
 ...
 *
 */
PRTE_EXPORT int prte_tree_serialize(prte_tree_item_t *start_item,
				      prte_buffer_t *buffer);

/**
 * De-serialize tree data
 *
 * @param buffer The prte buffer that is to be deserialized
 * @param start_item The item in the tree the data should be
 * deserialized into
 *
 * @returns Status of call PRTE_SUCCESS if everything worked
 *
 * This routine takes a bytestream that was created by the
 * prte_tree_serialize() function and deserializes it into the
 ...
 *
 */
PRTE_EXPORT int prte_tree_deserialize(prte_buffer_t *buffer,
 ...
 * @returns Success/Failure
 */
PRTE_EXPORT void * prte_tree_get_key(prte_tree_t *tree, prte_tree_item_t *item);
 ...
 * @returns Success/Failure
 */
PRTE_EXPORT int prte_tree_dup(prte_tree_t *from, prte_tree_t *to);
 ...
 */
PRTE_EXPORT int prte_tree_copy_subtree(prte_tree_t *from_tree, prte_tree_item_t *from_item,
 ...
 * @returns Tree item copy
 */
PRTE_EXPORT prte_tree_item_t *prte_tree_dup_item(prte_tree_t *base, prte_tree_item_t *from);
 ...
 */
PRTE_EXPORT int prte_tree_num_children(prte_tree_item_t *parent);
 ...
 */
PRTE_EXPORT int prte_tree_compare(prte_tree_t *left, prte_tree_t *right);
 ...
 *
 */
PRTE_EXPORT prte_tree_item_t *prte_tree_find_with(prte_tree_item_t *item,
                                                    void *key);
END_C_DECLS
