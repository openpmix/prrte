 ...
 * @file
 *
 * The prte_list_t interface is used to provide a generic
 * doubly-linked list container for PRTE.  It was inspired by (but
 * is slightly different than) the Stantard Template Library (STL)
 * std::list class.  One notable difference from std::list is that
 * when an prte_list_t is destroyed, all of the prte_list_item_t
 * objects that it contains are orphaned -- they are \em not
 * destroyed.
 *
 * The general idea is that prte_list_item_t objects can be put on an
 * prte_list_t.  Hence, you create a new type that derives from
 * prte_list_item_t; this new type can then be used with prte_list_t
 * containers.
 *
 * NOTE: prte_list_item_t instances can only be on \em one list at a
 * time.  Specifically, if you add an prte_list_item_t to one list,
 ...
 * have been warned.
 *
 * If PRTE_ENABLE_DEBUG is true, a bunch of checks occur, including
 * some spot checks for a debugging reference count in an attempt to
 * ensure that an prte_list_item_t is only one *one* list at a time.
 ...
 * threads may [erroneously] modify an prte_list_item_t concurrently.
 *
 ...
 */
PRTE_EXPORT PRTE_CLASS_DECLARATION(prte_list_t);
/**
 * \internal
 *
 * Base class for items that are put in list (prte_list_t) containers.
 */
PRTE_EXPORT PRTE_CLASS_DECLARATION(prte_list_item_t);

/**
 * \internal
 *
 * Struct of an prte_list_item_t
 */
struct prte_list_item_t {
    prte_object_t super;
    /**< Generic parent class for all PRTE objects */
    volatile struct prte_list_item_t *volatile prte_list_next;
    /**< Pointer to next list item */
    volatile struct prte_list_item_t *volatile prte_list_prev;
    /**< Pointer to previous list item */
    int32_t item_free;

#if PRTE_ENABLE_DEBUG
    /** Atomic reference count for debugging */
    prte_atomic_int32_t prte_list_item_refcount;
    /** The list this item belong to */
    volatile struct prte_list_t *prte_list_item_belong_to;
#endif
};
/**
 ...
 */
#define prte_list_get_next(item) \
    ((item) ? ((prte_list_item_t *) ((prte_list_item_t *) (item))->prte_list_next) : NULL)
 ...
 * @returns The next item in the list
 */
#define prte_list_get_prev(item) \
    ((item) ? ((prte_list_item_t *) ((prte_list_item_t *) (item))->prte_list_prev) : NULL)

/**
 * \internal
 *
 * Struct of an prte_list_t
 */
struct prte_list_t {
    prte_object_t super;
    /**< Generic parent class for all PRTE objects */
    prte_list_item_t prte_list_sentinel;
    /**< Head and tail item of the list */
    volatile size_t prte_list_length;
 ...
 *
 * The prte_list_t destructor doesn't release the items on the
 ...
 */
#define PRTE_LIST_DESTRUCT(list)                                    \
    do {                                                            \
        prte_list_item_t *it;                                       \
        if (1 == ((prte_object_t *) (list))->obj_reference_count) { \
            while (NULL != (it = prte_list_remove_first(list))) {   \
                PRTE_RELEASE(it);                                   \
            }                                                       \
        }                                                           \
        PRTE_DESTRUCT(list);                                        \
 ...
 *
 * This macro provides a simple way to loop over the items in an prte_list_t. It
 * is not safe to call prte_list_remove_item from within the loop.
 *
 * Example Usage:
 *
 * class_foo_t *foo;
 * PRTE_LIST_FOREACH(foo, list, class_foo_t) {
 *    do something(foo);
 * }
 */
#define PRTE_LIST_FOREACH(item, list, type)                         \
 ...
 * }
 */
#define PRTE_LIST_FOREACH_REV(item, list, type)                     \
    for (item = (type *) (list)->prte_list_sentinel.prte_list_prev; \
         item != (type *) &(list)->prte_list_sentinel;              \
         item = (type *) ((prte_list_item_t *) (item))->prte_list_prev)
 ...
 *
 * This macro provides a simple way to loop over the items in an prte_list_t. It
 * is safe to call prte_list_remove_item(list, item) from within the loop.
 *
 * Example Usage:
 *
 * class_foo_t *foo, *next;
 * prte_list_foreach_safe(foo, next, list, class_foo_t) {
 *    do something;
 *    prte_list_remove_item (list, (prte_list_item_t *) foo);
 * }
 */
#define PRTE_LIST_FOREACH_SAFE(item, next, list, type)                 \
    for (item = (type *) (list)->prte_list_sentinel.prte_list_next,    \
        next = (type *) ((prte_list_item_t *) (item))->prte_list_next; \
         item != (type *) &(list)->prte_list_sentinel;                 \
         item = next, next = (type *) ((prte_list_item_t *) (item))->prte_list_next)
 ...
 * @param[in] type Type of each list item
 *
 * This macro provides a simple way to loop over the items in an prte_list_t. If
 * is safe to call prte_list_remove_item(list, item) from within the loop.
 *
 * Example Usage:
 *
 ...
 * }
 */
#define PRTE_LIST_FOREACH_SAFE_REV(item, prev, list, type)             \
    for (item = (type *) (list)->prte_list_sentinel.prte_list_prev,    \
        prev = (type *) ((prte_list_item_t *) (item))->prte_list_prev; \
         item != (type *) &(list)->prte_list_sentinel;                 \
         item = prev, prev = (type *) ((prte_list_item_t *) (item))->prte_list_prev)
 ...
 */
static inline bool prte_list_is_empty(prte_list_t *list)
{
    return (list->prte_list_sentinel.prte_list_next == &(list->prte_list_sentinel) ? true : false);
}
 ...
 * prte_list_get_end() to ensure that the list is not empty.
 *
 * This is an inlined function in compilers that support inlining, so
 * it's usually a cheap operation.
 */
static inline prte_list_item_t *prte_list_get_first(prte_list_t *list)
{
    prte_list_item_t *item = (prte_list_item_t *) list->prte_list_sentinel.prte_list_next;
#if PRTE_ENABLE_DEBUG
    /* Spot check: ensure that the first item is only on one list */

    assert(1 == item->prte_list_item_refcount);
    assert(list == item->prte_list_item_belong_to);
#endif
 ...
 * should be compared against the returned value from
 * prte_list_get_begin() to ensure that the list is not empty.
 *
 * This is an inlined function in compilers that support inlining, so
 * it's usually a cheap operation.
 */
static inline prte_list_item_t *prte_list_get_last(prte_list_t *list)
{
    prte_list_item_t *item = (prte_list_item_t *) list->prte_list_sentinel.prte_list_prev;
#if PRTE_ENABLE_DEBUG
 ...
 * it's usually a cheap operation.
 */
static inline prte_list_item_t *prte_list_get_begin(prte_list_t *list)
{
    return &(list->prte_list_sentinel);
}
 ...
 * it's usually a cheap operation.
 */
static inline prte_list_item_t *prte_list_get_end(prte_list_t *list)
{
    return &(list->prte_list_sentinel);
}
 ...
 * the future, calling \c prte_list_splice or \c prte_list_join may
 * result in this function recomputing the list size, which would be
 * an O(N) operation.  If \c prte_list_splice or \c prte_list_join is
 * never called on the specified list, this function will always be
 * O(1).
 */
static inline size_t prte_list_get_size(prte_list_t *list)
{
#if PRTE_ENABLE_DEBUG && 0
    ...
    for (item = prte_list_get_first(list); item != prte_list_get_end(list);
         item = prte_list_get_next(item)) {
        check_len++;
    }

    if (check_len != list->prte_list_length) {
        fprintf(
            stderr,
            " Error :: prte_list_get_size - prte_list_length does not match actual list length\n");
        fflush(stderr);
        abort();
    }
#endif

    return list->prte_list_length;
}
 ...
 * caller -- they are responsible for PRTE_RELEASE()'ing it.
 *
 * If debugging is enabled (specifically, if --enable-debug was used
 * to configure PRTE), this is an O(N) operation because it checks
 * to see if the item is actually in the list first.
 *
 * This is an inlined function in compilers that support inlining, so
 * it's usually a cheap operation.
 */
static inline prte_list_item_t *prte_list_remove_item(prte_list_t *list, prte_list_item_t *item)
{
#if PRTE_ENABLE_DEBUG
    prte_list_item_t *item_ptr;
    bool found = false;

    /* check to see that the item is in the list */
    for (item_ptr = prte_list_get_first(list); item_ptr != prte_list_get_end(list);
         item_ptr = (prte_list_item_t *) (item_ptr->prte_list_next)) {
        if (item_ptr == (prte_list_item_t *) item) {
            found = true;
            break;
        }
    }
    if (!found) {
        fprintf(stderr, " Warning :: prte_list_remove_item - the item %p is not on the list %p \n",
                (void *) item, (void *) list);
        fflush(stderr);
        return (prte_list_item_t *) NULL;
    }
    ...
    item->prte_list_prev->prte_list_next = item->prte_list_next;

    /* reset previous pointer of next element */
    item->prte_list_next->prte_list_prev = item->prte_list_prev;
    ...
    PRTE_THREAD_ADD_FETCH32(&(item->prte_list_item_refcount), -1);
    assert(0 == item->prte_list_item_refcount);
    item->prte_list_item_belong_to = NULL;
#endif

    return (prte_list_item_t *) item->prte_list_prev;
}
 ...
 * The prte_list_item_t is not PRTE_RETAIN()'ed; it is assumed that
...
#if PRTE_ENABLE_DEBUG
#    define prte_list_append(l, i) _prte_list_append(l, i, __FILE__, __LINE__)
#else
#    define prte_list_append(l, i) _prte_list_append(l, i)
#endif /* PRTE_ENABLE_DEBUG */

static inline void _prte_list_append(prte_list_t *list, prte_list_item_t *item
#if PRTE_ENABLE_DEBUG
                                     ,
...
{
    prte_list_item_t *sentinel = &(list->prte_list_sentinel);
#if PRTE_ENABLE_DEBUG
    ...
    item->prte_list_prev = sentinel->prte_list_prev;

    /* reset previous pointer on current last element */
    sentinel->prte_list_prev->prte_list_next = item;

    /* reset new element's next pointer */
    item->prte_list_next = sentinel;

    /* reset the list's tail element previous pointer */
    sentinel->prte_list_prev = item;
 ...
 * list.  The prte_list_item_t is not PRTE_RETAIN()'ed; it is assumed
 * that "ownership" of the item is passed from the caller to the list.
 *
 * This is an inlined function in compilers that support inlining, so
 * it's usually a cheap operation.
 */
static inline void prte_list_prepend(prte_list_t *list, prte_list_item_t *item)
{
    prte_list_item_t *sentinel = &(list->prte_list_sentinel);
#if PRTE_ENABLE_DEBUG
    /* Spot check: ensure that this item is previously on no lists */
    ...
    item->prte_list_next = sentinel->prte_list_next;

    /* reset item's previous pointer */
    item->prte_list_prev = sentinel;

    /* reset previous first element's previous poiner */
    sentinel->prte_list_next->prte_list_prev = item;

    /* reset head's next pointer */
    sentinel->prte_list_next = item;
 ...
 * the caller; no calls to PRTE_RETAIN() or PRTE_RELEASE() are invoked.
 *
 * This is an inlined function in compilers that support inlining, so
 * it's usually a cheap operation.
 */
static inline prte_list_item_t *prte_list_remove_first(prte_list_t *list)
{
    ...
    */
    ...
    assert(1 == list->prte_list_sentinel.prte_list_next->prte_list_item_refcount);
#endif
    ...
    item = (prte_list_item_t *) list->prte_list_sentinel.prte_list_next;

    /* reset previous pointer of next item on the list */
    item->prte_list_next->prte_list_prev = item->prte_list_prev;

    /* reset the head next pointer */
    list->prte_list_sentinel.prte_list_next = item->prte_list_next;

#if PRTE_ENABLE_DEBUG
    assert(list == item->prte_list_item_belong_to);
    item->prte_list_item_belong_to = NULL;
    item->prte_list_prev = (prte_list_item_t *) NULL;
    item->prte_list_next = (prte_list_item_t *) NULL;
 ...
 * it's usually a cheap operation.
 */
static inline prte_list_item_t *prte_list_remove_last(prte_list_t *list)
{
    ...
    */
    ...
    assert(1 == list->prte_list_sentinel.prte_list_prev->prte_list_item_refcount);
#endif
    ...
    /* get item */
    item = (prte_list_item_t *) list->prte_list_sentinel.prte_list_prev;

    /* reset previous pointer on next to last pointer */
    item->prte_list_prev->prte_list_next = item->prte_list_next;

    /* reset tail's previous pointer */
    list->prte_list_sentinel.prte_list_prev = item->prte_list_prev;

#if PRTE_ENABLE_DEBUG
    assert(list == item->prte_list_item_belong_to);
    item->prte_list_next = item->prte_list_prev = (prte_list_item_t *) NULL;
 ...
 */
static inline void prte_list_insert_pos(prte_list_t *list, prte_list_item_t *pos,
                                        prte_list_item_t *item)
{
#if PRTE_ENABLE_DEBUG
    ...
    item->prte_list_next = pos;
    item->prte_list_prev = pos->prte_list_prev;

    /* splice into the list */
    pos->prte_list_prev->prte_list_next = item;
    pos->prte_list_prev = item;
 ...
 */
PRTE_EXPORT bool prte_list_insert(prte_list_t *list, prte_list_item_t *item, long long idx);
 ...
 * but valid after the call.  All pointers to \c prte_list_item_t
 * containers remain valid, including those that point to elements
 * in \c xlist.
 */
PRTE_EXPORT void prte_list_join(prte_list_t *thislist, prte_list_item_t *pos, prte_list_t *xlist);
 ...
 * be recomputed.
 */
PRTE_EXPORT void prte_list_splice(prte_list_t *thislist, prte_list_item_t *pos, prte_list_t *xlist,
                                  prte_list_item_t *first, prte_list_item_t *last);

/**
 * Comparison function for prte_list_sort(), below.
 *
 * @param a Pointer to a pointer to an prte_list_item_t.
 * Explanation below.
 * @param b Pointer to a pointer to an prte_list_item_t.
 * Explanation below.
 ...
 * prte_list_sort().  It is important to understand what
 * prte_list_sort() does before invoking qsort, so go read that
 * documentation first.
 *
 ...
 */
typedef int (*prte_list_item_compare_fn_t)(prte_list_item_t **a, prte_list_item_t **b);
 ...
 *    (prte_list_item_t*)'s in an array
 ...
 * a double de-reference to get to the actual prte_list_item_t (or
 * whatever the underlying type is).  See the documentation of
 * prte_list_item_compare_fn_t for an example).
 */
PRTE_EXPORT int prte_list_sort(prte_list_t *list, prte_list_item_compare_fn_t compare);
