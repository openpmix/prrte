...
{
    return (list->prte_list_sentinel.prte_list_next ==
        &(list->prte_list_sentinel) ? true : false);
}
...
static inline size_t prte_list_get_size(prte_list_t* list)
{
    ...
    for (item = prte_list_get_first(list) ;
         item != prte_list_get_end(list) ;
         item = prte_list_get_next(item)) {
        check_len++;
    }

    if (check_len != list->prte_list_length) {
        fprintf(stderr," Error :: prte_list_get_size - prte_list_length does not match actual list length\n");
        fflush(stderr);
        abort();
    }
#endif
 ...
 * it's usually a cheap operation.
 */
static inline prte_list_item_t *prte_list_remove_item
  (prte_list_t *list, prte_list_item_t *item)
{
#if PRTE_ENABLE_DEBUG
    ...
    for (item_ptr = prte_list_get_first(list);
            item_ptr != prte_list_get_end(list);
...
#if PRTE_ENABLE_DEBUG
#define prte_list_append(l,i) \
_prte_list_append(l,i,__FILE__,__LINE__)
#else
#define prte_list_append(l,i) \
_prte_list_append(l,i)
 ...
 * it's usually a cheap operation.
 */
static inline void prte_list_prepend(prte_list_t *list,
                                     prte_list_item_t *item)
{
    prte_list_item_t* sentinel = &(list->prte_list_sentinel);
#if PRTE_ENABLE_DEBUG
                                        ...
                                        prte_list_item_t *item)
{
   ...
   */
  PRTE_EXPORT bool prte_list_insert(prte_list_t *list, prte_list_item_t *item,
                                      long long idx);
     ...
     * in \c xlist.
     */
    PRTE_EXPORT void prte_list_join(prte_list_t *thislist, prte_list_item_t *pos,
                                      prte_list_t *xlist);
     ...
     * be recomputed.
     */
    PRTE_EXPORT void prte_list_splice(prte_list_t *thislist, prte_list_item_t *pos,
     ...
     */
    typedef int (*prte_list_item_compare_fn_t)(prte_list_item_t **a,
                                               prte_list_item_t **b);
