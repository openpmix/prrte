...
static void prte_graph_construct(prte_graph_t *graph)
{
    graph->adjacency_list = PRTE_NEW(prte_list_t);
...
static void prte_graph_destruct(prte_graph_t *graph)
{
    PRTE_LIST_RELEASE(graph->adjacency_list);
    graph->number_of_vertices = 0;
    graph->number_of_edges = 0;
}
...
{
    aj_list->vertex = NULL;
    aj_list->edges = PRTE_NEW(prte_list_t);
}
 ...
 * @param vertex
 */
static void delete_all_edges_conceded_to_vertex(prte_graph_t *graph, prte_graph_vertex_t *vertex)
{
    prte_adjacency_list_t *aj_list;
    prte_graph_edge_t *edge, *next;
...
void prte_graph_remove_vertex(prte_graph_t *graph, prte_graph_vertex_t *vertex)
{
 ...
 * @return int
 */
int prte_graph_get_order(prte_graph_t *graph)
{
 ...
 * @return int
 */
int prte_graph_get_size(prte_graph_t *graph)
{
 ...
 */
prte_graph_vertex_t *prte_graph_find_vertex(prte_graph_t *graph, void *vertex_data)
{
    prte_adjacency_list_t *aj_list;
 ...
 */
int prte_graph_get_graph_vertices(prte_graph_t *graph, prte_pointer_array_t *vertices_list)
{
    prte_adjacency_list_t *aj_list;
                                     ...
                                     prte_value_array_t *adjacents)
{
     ...
     */
    adj_list = (prte_adjacency_list_t *) vertex->in_adj_list;
    ...
    PRTE_LIST_FOREACH(edge, adj_list->edges, prte_graph_edge_t)
    {
        ...
        prte_value_array_append_item(adjacents, &distance_from);
    }
                        ...
                        prte_graph_vertex_t *vertex2)
{
    ...
    for (i = 0; i < items_in_distance_array; i++) {
        vertex_distance = prte_value_array_get_item(distance_array, i);
        if (vertex_distance->vertex == vertex2) {
            spf = vertex_distance->weight;
            break;
        }
    }
    PRTE_RELEASE(distance_array);
 ...
 */
void prte_graph_duplicate(prte_graph_t **dest, prte_graph_t *src)
{
    prte_adjacency_list_t *aj_list;
    prte_graph_vertex_t *vertex;
    prte_graph_edge_t *edge, *new_edge;
    ...
    *dest = PRTE_NEW(prte_graph_t);
    ...
    PRTE_LIST_FOREACH(aj_list, src->adjacency_list, prte_adjacency_list_t)
    {
        ...
        prte_graph_add_vertex(*dest, vertex);
    }
    /**
    ...
    PRTE_LIST_FOREACH(aj_list, src->adjacency_list, prte_adjacency_list_t)
    {
        ...
        PRTE_LIST_FOREACH(edge, aj_list->edges, prte_graph_edge_t)
        {
            ...
            prte_graph_add_edge(*dest, new_edge);
        }
    }
}
 ...
 * @param graph
 */
void prte_graph_print(prte_graph_t *graph)
{
    prte_adjacency_list_t *aj_list;
    prte_graph_edge_t *edge;
