...
#include "src/class/opal_hotel.h"
...
int main(int argc, char* argv[])
{
    ...
    if (0 > (rc = opal_init(&argc, &argv))) {
        fprintf(stderr, "prrte_hotel: couldn't init opal - error code %d\n", rc);
        return rc;
    }

    PRRTE_CONSTRUCT(&hotel, opal_hotel_t);
    opal_hotel_init(&hotel, NUM_RMS, opal_sync_event_base,
                    3000000, PRRTE_EV_SYS_HI_PRI, evict_cbfunc);
    ...
    for (i=0; i < NUM_RMS; i++) {
        if (PRRTE_SUCCESS != opal_hotel_checkin(&hotel,
                                               (void*)(&occupants[i]), &rm)) {
