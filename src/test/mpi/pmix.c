...
/*
 * To compile test:
 * mpicc -I$src_dir -I$src_dir/src/include -I$src_dir/src/include -I$src_dir/ompi/include -DOMPI_BUILDING=1 pmix.c -o pmix
 * To run test:
...
int main(int argc, char* argv[])
{
    ...
    int data = my_rank;
    t0 = get_timestamp();
    PRRTE_MODEX_SEND_VALUE(rc, PRRTE_PMIX_GLOBAL, "MY_RANK", &data, PRRTE_INT);
    t1 = get_timestamp();
    if (PRRTE_SUCCESS != rc) {
        DO_FINALIZE(rc, 1, "[%d] PRRTE_MODEX_SEND_STRING failed.\n", my_rank);
    }
    t2 = get_timestamp();
    opal_pmix.commit();
    ...
    for ( i = 0; i < numprocs; i++ ) {
        thisproc = procs[i];
        PRRTE_MODEX_RECV_VALUE(rc, "MY_RANK", &thisproc->super.proc_name, (void**)&ptr, PRRTE_INT);
        /* check return status and received data */
        if (PRRTE_SUCCESS != rc || i != recv_data) {
            rc = PRRTE_ERROR;
            DO_FINALIZE(rc, 1, "[%d] PRRTE_MODEX_RECV_VALUE failed from rank %d.\n", my_rank, i);
        }
    }
    t5 = get_timestamp();
