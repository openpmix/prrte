                           ...
                           void *cbdata)
{
    ...
    if (NULL != info) {
        PRRTE_LIST_FOREACH(val, info, opal_value_t) {
            if (0 == strcmp(val->key, PRRTE_PMIX_PROGRAMMING_MODEL) &&
                0 == strcmp(val->data.string, "OpenMP")) {
                goto cback;
            }
            if (PRRTE_STRING == val->type) {
  ...
  cback:
    ...
    PRRTE_PMIX_WAKEUP_THREAD(&thread_complete);
}
...
static void *mylib(void *ptr)
{
    ...
    PRRTE_PMIX_CONSTRUCT_LOCK(&thread_complete);

    /* declare that we are present and active */
    PRRTE_CONSTRUCT(&info, opal_list_t);
    kv = PRRTE_NEW(opal_value_t);
    kv->key = strdup(PRRTE_PMIX_PROGRAMMING_MODEL);
    kv->type = PRRTE_STRING;
    kv->data.string = strdup("OpenMP");
    opal_list_append(&info, &kv->super);
    kv = PRRTE_NEW(opal_value_t);
    kv->key = strdup(PRRTE_PMIX_MODEL_LIBRARY_NAME);
    kv->type = PRRTE_STRING;
    kv->data.string = strdup("foobar");
    opal_list_append(&info, &kv->super);
    kv = PRRTE_NEW(opal_value_t);
    kv->key = strdup(PRRTE_PMIX_MODEL_LIBRARY_VERSION);
    kv->type = PRRTE_STRING;
    kv->data.string = strdup("1.2.3.4");
    opal_list_append(&info, &kv->super);
    kv = PRRTE_NEW(opal_value_t);
    kv->key = strdup(PRRTE_PMIX_THREADING_MODEL);
    kv->type = PRRTE_STRING;
    ...
    if (opal_pmix.initialized()) {
        ...
        kv = PRRTE_NEW(opal_value_t);
        kv->key = strdup(PRRTE_PMIX_EVENT_NON_DEFAULT);
        kv->type = PRRTE_BOOL;
        kv->data.flag = true;
        opal_list_append(&info, &kv->super);
        ...
        ret = opal_pmix.notify_event(PRRTE_ERR_MODEL_DECLARED,
                                     &prrte_process_info.my_name,
                                     PRRTE_PMIX_RANGE_PROC_LOCAL, &info,
                                     opcbfunc, &lock);
        PRRTE_PMIX_WAIT_THREAD(&lock);
        PRRTE_PMIX_DESTRUCT_LOCK(&lock);
        PRRTE_LIST_DESTRUCT(&info);
    } else {
    ...
    PRRTE_CONSTRUCT(&directives, opal_list_t);
    kv = PRRTE_NEW(opal_value_t);
    kv->key = strdup(PRRTE_PMIX_EVENT_HDLR_NAME);
    kv->type = PRRTE_STRING;
    ...
    kv->type = PRRTE_INT;
    kv->data.integer = PRRTE_ERR_MODEL_DECLARED;
    opal_list_append(&info, &kv->super);
    ...
    PRRTE_PMIX_DESTRUCT_LOCK(&lock);
    PRRTE_LIST_DESTRUCT(&info);
    PRRTE_LIST_DESTRUCT(&directives);

    /* wait for the model callback */
    PRRTE_PMIX_WAIT_THREAD(&thread_complete);

    /* let's do a couple of operations just to verify we can,
     * starting with a query */
    PRRTE_CONSTRUCT(&info, opal_list_t);
    query = PRRTE_NEW(opal_pmix_query_t);
    opal_argv_append_nosize(&query->keys, PRRTE_PMIX_QUERY_NAMESPACES);
    ...
    /* Get something */
    opal_pmix.get(&prrte_process_info.my_name,
                  "WASSUP", NULL, &kv);
    if (NULL == kv) {
    ...
    PRRTE_CONSTRUCT(&info, opal_list_t);
    pdata = PRRTE_NEW(opal_pmix_pdata_t);
    pdata->proc = prrte_process_info.my_name;
    pdata->value.key = strdup("SOMETHING");
    opal_list_append(&info, &pdata->super);
    /* tell the call to wait for the data to be published */
    PRRTE_CONSTRUCT(&directives, opal_list_t);
    kv = PRRTE_NEW(opal_value_t);
    kv->key = strdup(PRRTE_PMIX_WAIT);
    kv->type = PRRTE_INT;
    kv->data.integer = 0;  // wait for all
    opal_list_append(&directives, &kv->super);

    if (PRRTE_SUCCESS != opal_pmix.lookup(&info, &directives)) {
...
int main(int argc, char* argv[])
{
    ...
    PRRTE_CONSTRUCT(&kv, opal_value_t);
    kv.key = strdup("WASSUP");
    kv.type = PRRTE_STRING;
    kv.data.string = strdup("nothing");
    opal_pmix.put(PRRTE_PMIX_LOCAL, &kv);
    PRRTE_DESTRUCT(&kv);
    ...
    fflush(stdout);
    if (PRRTE_SUCCESS == opal_hwloc_base_get_topology()) {
    ...
    printf("Hello, World, I am %d of %d [%d local peers]: get_cpubind: %d bitmap %s\n",
           rank, size, prrte_process_info.num_local_peers, rc,
           (NULL == bindings) ? "NULL" : bindings);

    /* publish something */
    PRRTE_CONSTRUCT(&list, opal_list_t);
    kptr = PRRTE_NEW(opal_value_t);
    kptr->key = strdup("SOMETHING");
    kptr->type = PRRTE_STRING;
    kptr->data.string = strdup("SILLY-THING");
    opal_list_append(&list, &kptr->super);
    opal_pmix.publish(&list);
    PRRTE_LIST_DESTRUCT(&list);
