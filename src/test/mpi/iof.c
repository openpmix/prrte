...
int main(int argc, char *argv[])
{
    ...
    msg = malloc(PRRTE_IOF_BASE_MSG_MAX);
    pos = 0;
    nbytes = 0;
    ...
    if (0 == rank) {
        while (0 != (msgsize = read(0, msg, PRRTE_IOF_BASE_MSG_MAX))) {
            ...
            if (msgsize > 0) {
                MPI_Bcast(msg, PRRTE_IOF_BASE_MSG_MAX, MPI_BYTE, 0, MPI_COMM_WORLD);
            }
            ++pos;
            nbytes += msgsize;
        }
        fprintf(stderr, "Rank %d: sending termination blob %d\n", rank, pos);
        memset(msg, 0, PRRTE_IOF_BASE_MSG_MAX);
        MPI_Bcast(msg, PRRTE_IOF_BASE_MSG_MAX, MPI_BYTE, 0, MPI_COMM_WORLD);
        ...
        while (1) {
            ...
            for (i=0; i < PRRTE_IOF_BASE_MSG_MAX; i++) {
                if (0 != msg[i]) {
                    done = false;
                    break;
                }
