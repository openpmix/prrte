...
int prte_hwloc_base_register(void)
{
    ...
    prte_mca_base_var_enum_create("hwloc memory bind failure action", hwloc_failure_action, &new_enum);
    ...
    (void) prte_mca_base_var_register("prte", "hwloc", "default", "binding_policy",
    ...
    prte_mca_base_var_register("prte", "hwloc", "default", "cpu_list",
...
PRTE_CLASS_INSTANCE(prte_hwloc_obj_data_t,
                   prte_object_t,
                   obj_data_const, NULL);
...
PRTE_CLASS_INSTANCE(prte_hwloc_summary_t,
                   prte_list_item_t,
                   sum_const, sum_dest);
...
PRTE_CLASS_INSTANCE(prte_hwloc_topo_data_t,
                   prte_object_t,
                   topo_data_const,
                   topo_data_dest);

PRTE_CLASS_INSTANCE(prte_rmaps_numa_node_t,
        prte_list_item_t,
        NULL,
        NULL);
...
int prte_hwloc_base_set_binding_policy(void *jdat, char *spec)
{
    ...
    if (0 < len) {
        if (0 == strncasecmp(myspec, "none", len)) {
            ...
            prte_show_help("help-prte-hwloc-base.txt", "invalid binding_policy", true, "binding", spec);
            free(myspec);
            return PRTE_ERR_BAD_PARAM;
        }
    }
