...
#define PRTE_GET_BINDING_POLICY(pol) \
    ((pol) & 0x0fff)
    ...
    do {                                                        \
        if (!PRTE_BINDING_POLICY_IS_SET((target))) {           \
            (target) = (pol) | (((target) & 0xf000) |           \
                                PRTE_BIND_IF_SUPPORTED);       \
        }                                                       \
    } while(0);

/* check if policy is set */
#define PRTE_BINDING_POLICY_IS_SET(pol) \
    ((pol) & 0x4000)
/* macro to detect if binding was qualified */
#define PRTE_BINDING_REQUIRED(n) \
    (!(PRTE_BIND_IF_SUPPORTED & (n)))
/* macro to detect if binding is forced */
#define PRTE_BIND_OVERLOAD_ALLOWED(n) \
    (PRTE_BIND_ALLOW_OVERLOAD & (n))
#define PRTE_BIND_ORDERED_REQUESTED(n) \
    (PRTE_BIND_ORDERED & (n))
 ...
 */
typedef enum {
    PRTE_HWLOC_BASE_MAP_NONE,
    PRTE_HWLOC_BASE_MAP_LOCAL_ONLY
} prte_hwloc_base_map_t;
 ...
 */
PRTE_EXPORT unsigned int prte_hwloc_base_get_npus(hwloc_topology_t topo,
                                                    bool use_hwthread_cpus,
 ...
 */
PRTE_EXPORT int prte_hwloc_base_report_bind_failure(const char *file,
                                                      int line,
                                                      const char *msg,
                                                      int rc);
...
PRTE_EXPORT int prte_hwloc_base_membind(prte_hwloc_base_memory_segment_t *segs,
 ...
 * 2[core 3]").
 */
PRTE_EXPORT char* prte_hwloc_base_cset2str(hwloc_cpuset_t cpuset,
                                             bool use_hwthread_cpus,
                                             hwloc_topology_t topo);


/* get the hwloc object that corresponds to the given processor id  and type */
PRTE_EXPORT hwloc_obj_t prte_hwloc_base_get_pu(hwloc_topology_t topo,
                                                 bool use_hwthread_cpus,
                                                 int lid);
...
PRTE_EXPORT char* prte_hwloc_base_get_location(char *locality,
                                                 hwloc_obj_type_t type,
                                                 unsigned index);

PRTE_EXPORT prte_hwloc_locality_t prte_hwloc_compute_relative_locality(char *loc1, char *loc2);

PRTE_EXPORT int prte_hwloc_base_topology_export_xmlbuffer(hwloc_topology_t topology, char **xmlpath, int *buflen);

PRTE_EXPORT int prte_hwloc_base_topology_set_flags (hwloc_topology_t topology, unsigned long flags, bool io);
