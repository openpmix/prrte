 ...
 */
const char *prte_plm_rsh_component_version_string =
  "PRTE rsh plm MCA component version " PRTE_VERSION;


static int rsh_component_register(void);
static int rsh_component_open(void);
static int rsh_component_query(prte_mca_base_module_t **module, int *priority);
static int rsh_component_close(void);
static int rsh_launch_agent_lookup(const char *agent_list, char *path);

/* Local variables */
static char *prte_plm_rsh_delay_string = NULL;
...
prte_plm_rsh_component_t prte_plm_rsh_component = {
    {
...
static int rsh_component_register(void)
{
    prte_mca_base_component_t *c = &prte_plm_rsh_component.super.base_version;
    int var_id;
    ...
    (void) prte_mca_base_var_register_synonym (var_id, "prte", "pls", NULL, "rsh_agent", PRTE_MCA_BASE_VAR_SYN_FLAG_DEPRECATED);
    (void) prte_mca_base_var_register_synonym (var_id, "prte", "prte", NULL, "rsh_agent", PRTE_MCA_BASE_VAR_SYN_FLAG_DEPRECATED);
    agent_var_id = var_id;
...
static int rsh_component_open(void)
{
    ...
    /* initialize globals */
    prte_plm_rsh_component.using_qrsh = false;
    prte_plm_rsh_component.using_llspawn = false;
    prte_plm_rsh_component.agent_argv = NULL;

    /* lookup parameters */
    if (prte_plm_rsh_component.num_concurrent <= 0) {
        prte_show_help("help-plm-rsh.txt", "concurrency-less-than-zero",
                       true, prte_plm_rsh_component.num_concurrent);
        prte_plm_rsh_component.num_concurrent = 1;
    }

    if (NULL != prte_plm_rsh_delay_string) {
        prte_plm_rsh_component.delay.tv_sec = strtol(prte_plm_rsh_delay_string, &ctmp, 10);
        if (ctmp == prte_plm_rsh_delay_string) {
            prte_plm_rsh_component.delay.tv_sec = 0;
        }
        if (':' == ctmp[0]) {
            prte_plm_rsh_component.delay.tv_nsec = 1000 * strtol (ctmp + 1, NULL, 10);
        }
    }
...
static int rsh_component_query(prte_mca_base_module_t **module, int *priority)
{
    ...
    ret = prte_mca_base_var_get_value(agent_var_id, NULL, &source, NULL);
    if (PRTE_SUCCESS != ret) {
        return ret;
    }
    if (PRTE_MCA_BASE_VAR_SOURCE_DEFAULT != source) {
    ...
    /* check for SGE */
    if (!prte_plm_rsh_component.disable_qrsh &&
        ...
        if (PRTE_SUCCESS != rsh_launch_agent_lookup("qrsh", tmp)) {
            /* can't be SGE */
        ...
        prte_plm_rsh_component.agent = tmp;
        prte_plm_rsh_component.using_qrsh = true;
        goto success;
    }

    /* otherwise, check for LoadLeveler */
    if (!prte_plm_rsh_component.disable_llspawn &&
        NULL != getenv("LOADL_STEP_ID")) {
        /* Search for llspawn in the users PATH */
        if (PRTE_SUCCESS != rsh_launch_agent_lookup("llspawn", NULL)) {
             prte_output_verbose(1, prte_plm_base_framework.framework_output,
        ...
        prte_plm_rsh_component.agent = strdup("llspawn");
        prte_plm_rsh_component.using_llspawn = true;
        goto success;
    }
  ...
  lookup:
    if (PRTE_SUCCESS != rsh_launch_agent_lookup(NULL, NULL)) {
  ...
  success:
    ...
    *priority = prte_plm_rsh_component.priority;
    *module = (prte_mca_base_module_t *) &prte_plm_rsh_module;
    return PRTE_SUCCESS;
}
 ...
 */
char **prte_plm_rsh_search(const char* agent_list, const char *path)
{
    int i, j;
    char *line, **lines;
    char **tokens, *tmp;
    ...
    if (NULL == agent_list && NULL == prte_plm_rsh_component.agent) {
        return NULL;
    }
    ...
    if (NULL == agent_list) {
        lines = prte_argv_split(prte_plm_rsh_component.agent, ':');
    } else {
...
static int rsh_launch_agent_lookup(const char *agent_list, char *path)
{
    ...
    PRTE_OUTPUT_VERBOSE((5, prte_plm_base_framework.framework_output,
                         ...
                         PRTE_NAME_PRINT(PRTE_PROC_MY_NAME),
                         (NULL == agent_list) ? prte_plm_rsh_component.agent : agent_list,
                         (NULL == path) ? "NULL" : path));
    if (NULL == (prte_plm_rsh_component.agent_argv = prte_plm_rsh_search(agent_list, path))) {
        return PRTE_ERR_NOT_FOUND;
    }

    /* if we got here, then one of the given agents could be found - the
     * complete path is in the argv[0] position */
    prte_plm_rsh_component.agent_path = strdup(prte_plm_rsh_component.agent_argv[0]);
    bname = prte_basename(prte_plm_rsh_component.agent_argv[0]);
    if (NULL == bname) {
        return PRTE_SUCCESS;
    }
    /* replace the initial position with the basename */
    free(prte_plm_rsh_component.agent_argv[0]);
    prte_plm_rsh_component.agent_argv[0] = bname;
    ...
    if (0 == strcmp(bname, "ssh")) {
        ...
        if (NULL != prte_xterm) {
            prte_argv_append_unique_nosize(&prte_plm_rsh_component.agent_argv, "-X");
             ...
             */
            for (i = 1; NULL != prte_plm_rsh_component.agent_argv[i]; ++i) {
                if (0 == strcasecmp("-x", prte_plm_rsh_component.agent_argv[i])) {
                    break;
                }
            }
            if (NULL == prte_plm_rsh_component.agent_argv[i]) {
                prte_argv_append_nosize(&prte_plm_rsh_component.agent_argv, "-x");
            }
        }
    }
    return PRTE_SUCCESS;
