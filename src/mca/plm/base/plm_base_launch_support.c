                                   ...
                                   prte_rml_tag_t tag, void *cbdata)
{
    ...
    while (PMIX_SUCCESS == ret) {
        ...
        // save the unpack ptr
        saveptr = data->unpack_ptr;

        // unpack the xml signature
        idx = 1;
        ret = PMIx_Data_unpack(NULL, data, &pbo, &idx, PMIX_BYTE_OBJECT);
        if (PMIX_SUCCESS != ret) {
            PMIX_ERROR_LOG(ret);
            prted_failed_launch = true;
            goto CLEANUP;
        ...
        pbo.size = 0;
        // restore the pointer
        data->unpack_ptr = saveptr;

        // see if we already have this topology
        found = false;
        ...
        if (!found) {
            // unpack the full topology
            idx = 1;
            ret = prte_topology_unpack(data, &topo);
            if (PMIX_SUCCESS != ret) {
                PMIX_ERROR_LOG(ret);
                prted_failed_launch = true;
                goto CLEANUP;
