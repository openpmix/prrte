...
- **`prte_schizo_base_parse_prte` / `_parse_pmix`** — shared scanners
  ...
  `_parse_prte` and `_parse_tune` call `prte_publish_mca_prefixes()`
  before anything else; without it, a command line carrying `--mca` fixed
  PMIx on its built-in table and PRRTE's own list was never read. The
  list is `prte_framework_names` (generated by `autogen.pl`), which holds
  parameter *prefixes*, not just frameworks: `grpcomm`, `rml`, `routed`
  and `oob` name live parameters but are not framework directories.
  `test/unit/schizo/test_mca_prefixes` is a program of its own because it
  must reach the pre-scan with nothing initialized.
