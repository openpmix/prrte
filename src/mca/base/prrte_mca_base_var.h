...
#include "src/mca/base/prrte_mca_base_var_group.h"
...
typedef enum {
    /** The default value */
    PRRTE_MCA_BASE_VAR_SOURCE_DEFAULT,
    /** The value came from the command line */
    PRRTE_MCA_BASE_VAR_SOURCE_COMMAND_LINE,
    /** The value came from the environment */
    PRRTE_MCA_BASE_VAR_SOURCE_ENV,
    /** The value came from a file */
    PRRTE_MCA_BASE_VAR_SOURCE_FILE,
    /** The value came a "set" API call */
    PRRTE_MCA_BASE_VAR_SOURCE_SET,
    /** The value came from the override file */
    PRRTE_MCA_BASE_VAR_SOURCE_OVERRIDE,

    /** Maximum source type */
    PRRTE_MCA_BASE_VAR_SOURCE_MAX
} prrte_mca_base_var_source_t;
...
typedef enum {
    ...
    /** The value of this variable must be set to a consistent value
        within a group */
    PRRTE_MCA_BASE_VAR_SCOPE_GROUP,
    /** The value of this variable must be set to the same value
        within a group */
    PRRTE_MCA_BASE_VAR_SCOPE_GROUP_EQ,
    /** The value of this variable must be set to a consistent value
        for all processes */
    PRRTE_MCA_BASE_VAR_SCOPE_ALL,
    /** The value of this variable must be set to the same value
        for all processes */
    PRRTE_MCA_BASE_VAR_SCOPE_ALL_EQ,
    PRRTE_MCA_BASE_VAR_SCOPE_MAX
} prrte_mca_base_var_scope_t;
...
typedef enum {
    ...
    PRRTE_INFO_LVL_9,
    PRRTE_INFO_LVL_MAX
} prrte_mca_base_var_info_lvl_t;

typedef enum {
    PRRTE_MCA_BASE_VAR_SYN_FLAG_DEPRECATED = 0x0001,
    PRRTE_MCA_BASE_VAR_SYN_FLAG_INTERNAL   = 0x0002
} prrte_mca_base_var_syn_flag_t;

typedef enum {
    /** Variable is internal (hidden from *_info/MPIT) */
    PRRTE_MCA_BASE_VAR_FLAG_INTERNAL     = 0x0001,
    /** Variable will always be the default value. Implies
        !MCA_BASE_VAR_FLAG_SETTABLE */
    PRRTE_MCA_BASE_VAR_FLAG_DEFAULT_ONLY = 0x0002,
    /** Variable can be set with mca_base_var_set() */
    PRRTE_MCA_BASE_VAR_FLAG_SETTABLE     = 0x0004,
    /** Variable is deprecated */
    PRRTE_MCA_BASE_VAR_FLAG_DEPRECATED   = 0x0008,
    /** Variable has been overridden */
    PRRTE_MCA_BASE_VAR_FLAG_OVERRIDE     = 0x0010,
    /** Variable may not be set from a file */
    PRRTE_MCA_BASE_VAR_FLAG_ENVIRONMENT_ONLY = 0x0020,
    ...
    PRRTE_MCA_BASE_VAR_FLAG_DWG          = 0x0040,
    /** Variable has a default value of "unset". Meaning to only
     * be set when the user explicitly asks for it */
    PRRTE_MCA_BASE_VAR_FLAG_DEF_UNSET    = 0x0080,
} prrte_mca_base_var_flag_t;
...
typedef union {
    ...
    /** double value */
    double lfval;
} prrte_mca_base_var_storage_t;


/**
 * Entry for holding information about an MCA variable.
 */
struct prrte_mca_base_var_t {
    ...
    prrte_mca_base_var_info_lvl_t mbv_info_lvl;

    /** Enum indicating the type of the variable (integer, string, boolean) */
    prrte_mca_base_var_type_t mbv_type;
    ...
    /** List of synonym names for this variable.  This *must* be a
        pointer (vs. a plain prrte_list_t) because we copy this whole
        struct into a new var for permanent storage
        (prrte_vale_array_append_item()), and the internal pointers in
        the prrte_list_t will be invalid when that happens.  Hence, we
        simply keep a pointer to an external prrte_list_t.  Synonyms
        are uncommon enough that this is not a big performance hit. */
    prrte_value_array_t mbv_synonyms;

    /** Variable flags */
    prrte_mca_base_var_flag_t  mbv_flags;

    /** Variable scope */
    prrte_mca_base_var_scope_t  mbv_scope;

    /** Source of the current value */
    prrte_mca_base_var_source_t mbv_source;
    ...
    prrte_mca_base_var_enum_t *mbv_enumerator;

    /** Bind value for this variable (0 - none) */
    int mbv_bind;

    /** Storage for this variable */
    prrte_mca_base_var_storage_t *mbv_storage;
 ...
 * Convenience typedef.
 */
typedef struct prrte_mca_base_var_t prrte_mca_base_var_t;
 ...
 */
PRRTE_EXPORT PRRTE_CLASS_DECLARATION(prrte_mca_base_var_t);

/**
 * Initialize the MCA variable system.
 *
 * @retval PRRTE_SUCCESS
 *
 * This function initalizes the MCA variable system.  It is
 * invoked internally (by mca_base_open()) and is only documented
 * here for completeness.
 */
PRRTE_EXPORT int prrte_mca_base_var_init(void);
PRRTE_EXPORT void prrte_mca_base_var_finalize (void);
 ...
 * @retval PRRTE_ERR_OUT_OF_RESOURCE Upon failure to allocate memory.
 * @retval PRRTE_ERROR Upon failure to register the variable.
 *
 ...
 * successfully.
 */
PRRTE_EXPORT int prrte_mca_base_var_register (const char *project_name, const char *framework_name,
                                         const char *component_name, const char *variable_name,
                                         const char *description, prrte_mca_base_var_type_t type,
                                         prrte_mca_base_var_enum_t *enumerator, int bind, prrte_mca_base_var_flag_t flags,
 ...
 * its framework.
 */
PRRTE_EXPORT int prrte_mca_base_component_var_register (const prrte_mca_base_component_t *component,
 ...
 */
PRRTE_EXPORT int prrte_mca_base_framework_var_register (const prrte_mca_base_framework_t *framework,
 ...
 * @returns PRRTE_ERR_BAD_VAR If synonym_for does not reference a valid
 * variable.
 * @returns PRRTE_ERR_OUT_OF_RESOURCE If memory could not be allocated.
 * @returns PRRTE_ERROR For all other errors.
 *
 ...
 */
PRRTE_EXPORT int prrte_mca_base_var_register_synonym (int synonym_for, const char *project_name,
 ...
 */
PRRTE_EXPORT int prrte_mca_base_var_deregister(int vari);
 ...
 *
 * @return PRRTE_ERROR Upon failure.  The contents of value are
 * undefined.
 * @return PRRTE_SUCCESS Upon success. value (if not NULL) will be filled
 ...
 */
PRRTE_EXPORT int prrte_mca_base_var_get_value (int vari, const void *value,
 ...
 * @retval PRRTE_ERR_PERM If the variable is not settable.
 * @retval PRRTE_ERR_BAD_PARAM If the variable does not exist or has
 * been deregistered.
 * @retval PRRTE_ERROR On other error.
 *
 ...
 * settable.
 */
PRRTE_EXPORT int prrte_mca_base_var_set_value (int vari, const void *value, size_t size,
                                          prrte_mca_base_var_source_t source,
                                          const char *source_file);
 ...
 */
PRRTE_EXPORT int prrte_mca_base_var_env_name(const char *param_name,
                                        char **env_name);
 ...
 *
 * @retval PRRTE_ERROR If the variable was not found.
 ...
 */
PRRTE_EXPORT int prrte_mca_base_var_find (const char *project_name,
 ...
 */
PRRTE_EXPORT int prrte_mca_base_var_find_by_name (const char *full_name, int *vari);
 ...
 *
 * This function will print an prrte_show_help() message and return
 * PRRTE_ERR_BAD_VAR if it finds that the two variables both have
 ...
 * not default values).
 *
 * Note that prrte_show_help() allows itself to be hooked, so if this
 * happens after the aggregated prrte_show_help() system is
 * initialized, the messages will be aggregated (w00t).
 *
 * @returns PRRTE_ERR_BAD_VAR if the two variables have sources that
 * are not MCA_BASE_VAR_SOURCE_DEFAULT.
 * @returns PRRTE_SUCCESS otherwise.
 */
PRRTE_EXPORT int prrte_mca_base_var_check_exclusive (const char *project,
                                                const char *type_a,
 ...
 *
 * @returns PRRTE_SUCCESS If the flags are set successfully.
 * @returns PRRTE_ERR_BAD_PARAM If the variable is not registered.
 * @returns PRRTE_ERROR Otherwise
 */
PRRTE_EXPORT int prrte_mca_base_var_set_flag(int vari, prrte_mca_base_var_flag_t flag,
                                        bool set);
 ...
 */
PRRTE_EXPORT int prrte_mca_base_var_get_count (void);
 ...
 */
PRRTE_EXPORT int prrte_mca_base_var_build_env(char ***env, int *num_env,
                                         bool internal);

typedef enum {
    /* Dump human-readable strings */
    PRRTE_MCA_BASE_VAR_DUMP_READABLE = 0,
    /* Dump easily parsable strings */
    PRRTE_MCA_BASE_VAR_DUMP_PARSABLE = 1,
    /* Dump simple name=value string */
    PRRTE_MCA_BASE_VAR_DUMP_SIMPLE   = 2
} prrte_mca_base_var_dump_type_t;
 ...
 */
PRRTE_EXPORT int prrte_mca_base_var_dump(int vari, char ***out, prrte_mca_base_var_dump_type_t output_type);
...
extern char *prrte_mca_base_env_list_sep;
extern char *prrte_mca_base_env_list_internal;

END_C_DECLS

#endif /* PRRTE_MCA_BASE_VAR_H */
