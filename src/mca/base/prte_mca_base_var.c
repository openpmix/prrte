...
static int register_variable (const char *project_name, const char *framework_name,
                              ...
                              prte_mca_base_var_scope_t scope, int synonym_for,
                              void *storage)
{
    ...
    if (0 > var_index) {
        ...
        if (scope < PRTE_MCA_BASE_VAR_SCOPE_LOCAL || (flags & PRTE_MCA_BASE_VAR_FLAG_DEFAULT_ONLY)) {
            if ((flags & PRTE_MCA_BASE_VAR_FLAG_DEFAULT_ONLY) && (flags & PRTE_MCA_BASE_VAR_FLAG_SETTABLE)) {
                prte_show_help("help-prte-mca-var.txt", "invalid-flag-combination",
                               true, "PRTE_MCA_BASE_VAR_FLAG_DEFAULT_ONLY", "PRTE_MCA_BASE_VAR_FLAG_SETTABLE");
                return PRTE_ERROR;
            }
        ...
        prte_hash_table_set_value_ptr (&prte_mca_base_var_index_hash, var->mbv_full_name, strlen (var->mbv_full_name),
        ...
        if (0 != compare_strings(framework_name, group->group_framework) ||
            ...
            prte_show_help("help-prte-mca-var.txt", "var-name-conflict",
...
#if PRTE_ENABLE_DEBUG
            prte_show_help("help-prte-mca-var.txt",
    ...
    if (!original) {
        ...
        if ((PRTE_MCA_BASE_VAR_TYPE_STRING == type || PRTE_MCA_BASE_VAR_TYPE_VERSION_STRING == type) && NULL != ((char **)storage)[0]) {
...
static int var_set_from_env (prte_mca_base_var_t *var, prte_mca_base_var_t *original)
{
    ...
    if (PRTE_MCA_BASE_VAR_SOURCE_OVERRIDE == original->mbv_source) {
        if (!prte_mca_base_var_suppress_override_warning) {
            prte_show_help("help-prte-mca-var.txt", "overridden-param-set",
                           true, var_full_name);
        }
 ...
 */
static int var_set_from_file (prte_mca_base_var_t *var, prte_mca_base_var_t *original, prte_list_t *file_values)
{
    const char *var_full_name = var->mbv_full_name;
    const char *var_long_name = var->mbv_long_name;
    bool deprecated = PRTE_VAR_IS_DEPRECATED(var[0]);
    ...
    PRTE_LIST_FOREACH(fv, file_values, prte_mca_base_var_file_value_t) {
        ...
        if (PRTE_MCA_BASE_VAR_FLAG_ENVIRONMENT_ONLY & original->mbv_flags) {
            prte_show_help("help-prte-mca-var.txt", "environment-only-param",
...
static char *source_name(prte_mca_base_var_t *var)
{
    ...
    if (PRTE_MCA_BASE_VAR_SOURCE_FILE == var->mbv_source || PRTE_MCA_BASE_VAR_SOURCE_OVERRIDE == var->mbv_source) {
