 ...
 */
int prrte_mca_base_cmd_line_setup(prrte_cmd_line_t *cmd)
{
    int ret = PRRTE_SUCCESS;

    ret = prrte_cmd_line_make_opt3(cmd, '\0', PRRTE_MCA_CMD_LINE_ID, PRRTE_MCA_CMD_LINE_ID, 2,
                                  "Pass context-specific MCA parameters; they are considered global if --g"PRRTE_MCA_CMD_LINE_ID" is not used and only one context is specified (arg0 is the parameter name; arg1 is the parameter value)");
    if (PRRTE_SUCCESS != ret) {
        return ret;
    }

    ret = prrte_cmd_line_make_opt3(cmd, '\0', "g"PRRTE_MCA_CMD_LINE_ID, "g"PRRTE_MCA_CMD_LINE_ID, 2,
    ...
    {
        prrte_cmd_line_init_t entry =
            {"prrte_mca_base_envar_file_prefix", '\0', "tune", NULL, 1,
             NULL, PRRTE_CMD_LINE_TYPE_STRING,
             "Application profile options file list for PRRTE",
 ...
 */
int prrte_mca_base_cmd_line_process_args(prrte_cmd_line_t *cmd,
                                         char ***context_env, char ***global_env)
{
    ...
    if (!prrte_cmd_line_is_taken(cmd, PRRTE_MCA_CMD_LINE_ID) &&
        !prrte_cmd_line_is_taken(cmd, "g"PRRTE_MCA_CMD_LINE_ID)) {
        return PRRTE_SUCCESS;
    }

    /* Handle app context-specific parameters */

    num_insts = prrte_cmd_line_get_ninsts(cmd, PRRTE_MCA_CMD_LINE_ID);
    params = values = NULL;
    for (i = 0; i < num_insts; ++i) {
        if (PRRTE_SUCCESS != (rc = process_arg(prrte_cmd_line_get_param(cmd, PRRTE_MCA_CMD_LINE_ID, i, 0),
                                               prrte_cmd_line_get_param(cmd, PRRTE_MCA_CMD_LINE_ID, i, 1),
                                               &params, &values))) {
            return rc;
        }
    }
    if (NULL != params) {
        ...
        prrte_argv_free(params);
        prrte_argv_free(values);
    }

    /* Handle global parameters */

    num_insts = prrte_cmd_line_get_ninsts(cmd, "g"PRRTE_MCA_CMD_LINE_ID);
    params = values = NULL;
    for (i = 0; i < num_insts; ++i) {
        if (PRRTE_SUCCESS != (rc = process_arg(prrte_cmd_line_get_param(cmd, "g"PRRTE_MCA_CMD_LINE_ID, i, 0),
                                               prrte_cmd_line_get_param(cmd, "g"PRRTE_MCA_CMD_LINE_ID, i, 1),
                                               &params, &values))) {
            return rc;
        }
    }
                       ...
                       char ***params, char ***values)
{
    ...
    /* If we didn't already have an value for the same param, save
       this one away */
    prrte_argv_append_nosize(params, param);
    prrte_argv_append_nosize(values, p1);
    free(p1);
...
static void add_to_env(char **params, char **values, char ***env)
{
    ...
    /* Loop through all the args that we've gotten and make env
       vars of the form PRRTE_MCA_PREFIX*=value. */

    for (i = 0; NULL != params && NULL != params[i]; ++i) {
        (void) prrte_mca_base_var_env_name (params[i], &name);
        prrte_setenv(name, values[i], true, env);
        free(name);
    }
}

void prrte_mca_base_cmd_line_wrap_args(char **args)
{
    int i;
    char *tstr;

    for (i=0; NULL != args && NULL != args[i]; i++) {
        if (0 == strcmp(args[i], "-"PRRTE_MCA_CMD_LINE_ID) ||
            0 == strcmp(args[i], "--"PRRTE_MCA_CMD_LINE_ID)) {
