 ...
 */
int
prrte_mca_base_component_compare_priority(prrte_mca_base_component_priority_list_item_t *a,
                                          prrte_mca_base_component_priority_list_item_t *b)
{
  ...
  if (a->cpli_priority > b->cpli_priority) {
    ...
    return prrte_mca_base_component_compare(a->super.cli_component,
                                            b->super.cli_component);
  }
}


int prrte_mca_base_component_compare(const prrte_mca_base_component_t* aa,
                                     const prrte_mca_base_component_t* bb)
{
    int val;

    val = strncmp(aa->mca_type_name, bb->mca_type_name,
                  PRRTE_MCA_BASE_MAX_TYPE_NAME_LEN);
    if (val != 0) {
      return -val;
    }

    val = strncmp(aa->mca_component_name, bb->mca_component_name,
                      PRRTE_MCA_BASE_MAX_COMPONENT_NAME_LEN);
    if (val != 0) {
      return -val;
    }
 ...
 */
int prrte_mca_base_component_compatible(const prrte_mca_base_component_t* aa,
                                        const prrte_mca_base_component_t* bb)
{
    int val;
 ...
 */
char * prrte_mca_base_component_to_string(const prrte_mca_base_component_t *a) {
    char * str = NULL;
    if(0 > prrte_asprintf(&str, "%s.%s.%d.%d", a->mca_type_name,
