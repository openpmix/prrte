...
| `prte_state_base_notify_data_server` | Tells the data server a job has ended, naming the lifetime that ended (`PMIX_PERSIST_APP`) so it drops the nspace's published data that was not to outlive it. **The master's call**, made when the job is over everywhere. Not gated on an external data server: the built-in one needs telling too, and while it was gated nothing was ever reclaimed from it. Sends to **two** stores where they differ — the global one (always at the master's rank, whether it holds the data or relays to another DVM) and this process's own, which is where a `PMIX_RANGE_LOCAL` publish lives. The global send is unconditional: the old "nobody local to us published" skip is a fact about our *own* store, and on the master it is true precisely when the publishers were on other nodes. |
| `prte_state_base_notify_local_data_server` | The daemon's call, made from `state_prted.c`'s `job_teardown()` when the procs of a job that **this daemon hosted** have all terminated. Purges our own store and nothing else. A daemon's share of a job finishing is not a lifetime ending — the job may be running on every other node — so sending the DVM-wide purge from there took the whole namespace's data out of the master's store while other nodes were still publishing into it. |
