 ...
 */
static int prrte_odls_default_launch_local_procs(prrte_buffer_t *data);
static int prrte_odls_default_kill_local_procs(prrte_pointer_array_t *procs);
static int prrte_odls_default_signal_local_procs(const prrte_process_name_t *proc, int32_t signal);
static int prrte_odls_default_restart_proc(prrte_proc_t *child);
 ...
 * Module
 */
prrte_odls_base_module_t prrte_odls_default_module = {
    .get_add_procs_data = prrte_odls_base_default_get_add_procs_data,
    .launch_local_procs = prrte_odls_default_launch_local_procs,
    .kill_local_procs = prrte_odls_default_kill_local_procs,
    .signal_local_procs = prrte_odls_default_signal_local_procs,
    .restart_proc = prrte_odls_default_restart_proc
};
...
int prrte_odls_default_kill_local_procs(prrte_pointer_array_t *procs)
{
    int rc;
...
static int do_child(prrte_odls_spawn_caddy_t *cd, int write_fd)
{
    ...
    if (NULL != cd->child) {
        /* setup stdout/stderr so that any error messages that we
           ...
           happened
        */
        if (PRRTE_FLAG_TEST(cd->jdata, PRRTE_JOB_FLAG_FORWARD_OUTPUT)) {
            if (PRRTE_SUCCESS != (i = prrte_iof_base_setup_child(&cd->opts, &cd->env))) {
                PRRTE_ERROR_LOG(i);
                send_error_show_help(write_fd, 1,
                                     "help-prrte-odls-default.txt",
                                     "iof setup failed",
    ...
    send_error_show_help(write_fd, 1,
                         "help-prrte-odls-default.txt", "execve error",
                         prrte_process_info.nodename, dir, cd->app->app, strerror(errno));
    /* Does not return */
}
...
static int do_parent(prrte_odls_spawn_caddy_t *cd, int read_fd)
{
    ...
    while (1) {
        ...
        if (msg.file_str_len > 0) {
            ...
            if (PRRTE_SUCCESS != rc) {
                prrte_show_help("help-prrte-odls-default.txt", "syscall fail",
                               true,
                               prrte_process_info.nodename, cd->app->app,
...
static int odls_default_fork_local_proc(void *cdptr)
{
    ...
    if (pipe(p) < 0) {
        ...
        if (NULL != child) {
            ...
            child->exit_code = PRRTE_ERR_SYS_LIMITS_PIPES;
        }
    ...
    if (pid < 0) {
        ...
        if (NULL != child) {
            child->state = PRRTE_PROC_STATE_FAILED_TO_START;
            child->exit_code = PRRTE_ERR_SYS_LIMITS_CHILDREN;
        }
...
int prrte_odls_default_launch_local_procs(prrte_buffer_t *data)
{
    int rc;
    prrte_jobid_t job;
    ...
    PRRTE_ACTIVATE_LOCAL_LAUNCH(job, odls_default_fork_local_proc);
...
static int send_signal(pid_t pd, int signal)
{
    ...
    if (prrte_odls_globals.signal_direct_children_only) {
        pid = pd;
    } else {
#if HAVE_SETPGID
...
static int prrte_odls_default_restart_proc(prrte_proc_t *child)
{
    ...
    if (PRRTE_SUCCESS != (rc = prrte_odls_base_default_restart_proc(child, odls_default_fork_local_proc))) {
        PRRTE_OUTPUT_VERBOSE((2, prrte_odls_base_framework.framework_output,
