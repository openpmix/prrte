...
{
    pmix_list_t *local_list = &mca_oob_tcp_component.local_ifs, *remote_list;
    ...
    while (!connected) {
        ...
        ptr = mca_oob_tcp_component.local_ifs.pmix_list_sentinel.pmix_list_next;
        for (i = 0; i < best_i; i++) {
            ptr = ptr->pmix_list_next;
        }
        intf = (pmix_pif_t *) ptr;
    ...
    if (!connected) {
         ...
         */
        PRTE_ACTIVATE_TCP_CMP_OP(peer, mca_oob_tcp_component_failed_to_connect);
...
int prte_oob_tcp_peer_recv_connect_ack(prte_oob_tcp_peer_t *pr, int sd, prte_oob_tcp_hdr_t *dhdr)
{
    ...
    if (NULL == peer) {
        ...
        if (NULL == peer) {
            ...
            pmix_list_append(&mca_oob_tcp_component.peers, &peer->super);
        }
    } else {
     ...
     */
    PRTE_ACTIVATE_TCP_CMP_OP(peer, mca_oob_tcp_component_set_module);
...
void prte_oob_tcp_peer_close(prte_oob_tcp_peer_t *peer)
{
     ...
     */
    PRTE_ACTIVATE_TCP_CMP_OP(peer, mca_oob_tcp_component_lost_connection);
