...
#    if defined(TCP_KEEPALIVE)
    /* set the idle time */
    if (setsockopt(sd, IPPROTO_TCP, TCP_KEEPALIVE, &mca_oob_tcp_component.keepalive_time,
                   sizeof(mca_oob_tcp_component.keepalive_time))
        < 0) {
        prte_output_verbose(5, prte_oob_base_framework.framework_output,
...
#    elif defined(TCP_KEEPIDLE)
    /* set the idle time */
    if (setsockopt(sd, IPPROTO_TCP, TCP_KEEPIDLE, &mca_oob_tcp_component.keepalive_time,
                   sizeof(mca_oob_tcp_component.keepalive_time))
        < 0) {
        prte_output_verbose(5, prte_oob_base_framework.framework_output,
...
#    if defined(TCP_KEEPINTVL)
    ...
    if (setsockopt(sd, IPPROTO_TCP, TCP_KEEPINTVL, &mca_oob_tcp_component.keepalive_intvl,
                   sizeof(mca_oob_tcp_component.keepalive_intvl))
        < 0) {
        prte_output_verbose(5, prte_oob_base_framework.framework_output,
...
#    if defined(TCP_KEEPCNT)
    /* set the miss rate */
    if (setsockopt(sd, IPPROTO_TCP, TCP_KEEPCNT, &mca_oob_tcp_component.keepalive_probes,
                   sizeof(mca_oob_tcp_component.keepalive_probes))
        < 0) {
        prte_output_verbose(5, prte_oob_base_framework.framework_output,
...
void prte_oob_tcp_set_socket_options(int sd)
{
    ...
    if (mca_oob_tcp_component.tcp_sndbuf > 0
        && setsockopt(sd, SOL_SOCKET, SO_SNDBUF, (char *) &mca_oob_tcp_component.tcp_sndbuf,
                      sizeof(int))
               < 0) {
        prte_output_verbose(5, prte_oob_base_framework.framework_output,
    ...
    if (mca_oob_tcp_component.tcp_rcvbuf > 0
        && setsockopt(sd, SOL_SOCKET, SO_RCVBUF, (char *) &mca_oob_tcp_component.tcp_rcvbuf,
                      sizeof(int))
               < 0) {
        prte_output_verbose(5, prte_oob_base_framework.framework_output,
...
prte_oob_tcp_peer_t *prte_oob_tcp_peer_lookup(const pmix_proc_t *name)
{
    ...
    PMIX_LIST_FOREACH(peer, &mca_oob_tcp_component.peers, prte_oob_tcp_peer_t)
    {
