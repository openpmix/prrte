...
int
prrte_ess_alps_get_first_rank_on_node(int *first_rank)
{
    int alps_status = 0;
    uint64_t apid;
    size_t alps_count;
    ...
    if (0 != ret) {
        PRRTE_OUTPUT_VERBOSE((20, prrte_ess_base_framework.framework_output,
                             "%s ess:alps: alps_app_lli_lock returned %d",
                             PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME), lli_ret));
    ...
    if (1 != place_ret) {
        PRRTE_OUTPUT_VERBOSE((20, prrte_ess_base_framework.framework_output,
                             ...
                             PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME), place_ret, strerror(errno)));
        ret = PRRTE_ERROR;
        goto fn_exit;
    }
    ...
    PRRTE_OUTPUT_VERBOSE((2, prrte_ess_base_framework.framework_output,
                           "%s ess:alps: alps_get_placement_info returned %d first pe on node is %d",
                            PRRTE_NAME_PRINT(PRRTE_PROC_MY_NAME), place_ret, orted_layout.firstPe));
 ...
 */
int
prrte_ess_alps_sync_start(void)
{
    int ret = PRRTE_SUCCESS;
    int lli_ret = 0;
    int alps_status = 0;
...
int
prrte_ess_alps_sync_complete(void)
{
    int ret = PRRTE_SUCCESS;
    int lli_ret = 0;
    int alps_status = 0;
