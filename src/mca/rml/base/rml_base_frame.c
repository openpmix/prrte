...
static int prte_rml_base_register(pmix_mca_base_register_flag_t flags)
{
...
static int prte_rml_base_close(void)
{
    ...
    return pmix_mca_base_framework_components_close(&prte_rml_base_framework, NULL);
}

static int prte_rml_base_open(pmix_mca_base_open_flag_t flags)
{
    /* Initialize globals */
    ...
    return pmix_mca_base_framework_components_open(&prte_rml_base_framework, flags);
}

PMIX_MCA_BASE_FRAMEWORK_DECLARE(prte, rml, "PRTE Run-Time Messaging Layer", prte_rml_base_register,
...
int prte_rml_base_select(void)
{
     ...
     */
    if (PRTE_SUCCESS
        != pmix_mca_base_select("rml", prte_rml_base_framework.framework_output,
...
PMIX_CLASS_INSTANCE(prte_self_send_xfer_t, pmix_object_t, xfer_cons, xfer_des);
