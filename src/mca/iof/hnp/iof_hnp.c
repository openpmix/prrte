...
static int init(void)
{
    ...
    PMIX_CONSTRUCT(&mca_iof_hnp_component.procs, pmix_list_t);
...
static int push_stdin(const pmix_proc_t *dst_name, uint8_t *data, size_t sz)
{
    ...
    /* do we already have this process in our list? */
    proct = NULL;
    PMIX_LIST_FOREACH(pptr, &mca_iof_hnp_component.procs, prte_iof_proc_t)
    {
    ...
    PMIX_LIST_FOREACH(proct, &mca_iof_hnp_component.procs, prte_iof_proc_t)
    {
        if (PMIX_CHECK_PROCID(&proct->name, peer)) {
            ...
            if (NULL == proct->stdinev && NULL == proct->revstdout && NULL == proct->revstderr) {
                pmix_list_remove_item(&mca_iof_hnp_component.procs, &proct->super);
                PMIX_RELEASE(proct);
            }
            break;
        }
...
static void hnp_complete(const prte_job_t *jdata)
{
    ...
    PMIX_LIST_FOREACH_SAFE(proct, next, &mca_iof_hnp_component.procs, prte_iof_proc_t)
    {
...
static int finalize(void)
{
    PMIX_DESTRUCT(&mca_iof_hnp_component.procs);
    return PRTE_SUCCESS;
}
