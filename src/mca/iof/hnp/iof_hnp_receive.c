                       ...
                       prte_rml_tag_t tag, void *cbdata)
{
    ...
    if (PRTE_IOF_XON & stream) {
        ...
        if (NULL != prte_mca_iof_hnp_component.stdinev && !prte_job_term_ordered
            && !prte_mca_iof_hnp_component.stdinev->active) {
            PRTE_IOF_READ_ACTIVATE(prte_mca_iof_hnp_component.stdinev);
        }
        goto CLEAN_RETURN;
    } else if (PRTE_IOF_XOFF & stream) {
        /* stop the stdin read event */
        if (NULL != prte_mca_iof_hnp_component.stdinev && !prte_mca_iof_hnp_component.stdinev->active) {
            prte_event_del(prte_mca_iof_hnp_component.stdinev->ev);
            prte_mca_iof_hnp_component.stdinev->active = false;
        }
        goto CLEAN_RETURN;
    }
