...
int
prte_iof_base_setup_prefork(prte_iof_base_io_conf_t *opts)
{
    int ret = -1;
    ...
    if (opts->usepty) {
         ...
         */
        ret = prte_openpty(&(opts->p_stdout[0]), &(opts->p_stdout[1]),
...
int
prte_iof_base_setup_child(prte_iof_base_io_conf_t *opts, char ***env)
{
    int ret;
    ...
    if(opts->p_stderr[1] != fileno(stderr)) {
        if( !prte_iof_base.redirect_app_stderr_to_stdout ) {
            ...
            if (ret < 0) return PRTE_ERR_PIPE_SETUP_FAILURE;
                                     ...
                                     prte_iof_proc_t *proct)
{
    ...
    if (prte_get_attribute(&jobdat->attributes, PRTE_JOB_OUTPUT_TO_DIRECTORY, (void**)&dirname, PMIX_STRING) &&
        ...
        if (NULL != proct->revstdout && NULL == proct->revstdout->sink) {
            ...
            PRTE_IOF_SINK_DEFINE(&proct->revstdout->sink, dst_name,
                                 fdout, PRTE_IOF_STDOUT,
        ...
        if (NULL != proct->revstderr && NULL == proct->revstderr->sink) {
            ...
            if (prte_get_attribute(&jobdat->attributes, PRTE_JOB_MERGE_STDERR_STDOUT, NULL, PMIX_BOOL)) {
                ...
                /* define a sink to that file descriptor */
                PRTE_IOF_SINK_DEFINE(&proct->revstderr->sink, dst_name,
                                     fdout, PRTE_IOF_STDERR,
                                     prte_iof_base_write_handler);
            }
        }
        return PRTE_SUCCESS;
