...
FAILED:
    /* We ran out of descriptors partway through. Hand back the ones we did
     * get: nobody else knows about them (the caller only ever reads these
     * fields after a success), so leaking them here permanently consumes
     * descriptors in a daemon that is already at its limit - which is what
     * turns one failed launch into every later one failing too.
     */
