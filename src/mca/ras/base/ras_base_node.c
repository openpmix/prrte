...
int prte_ras_base_node_insert(prte_list_t* nodes, prte_job_t *jdata)
{
     ...
     */
    if (PRTE_SUCCESS != (rc = prte_pointer_array_set_size(prte_node_pool, num_nodes * prte_ras_base.multiplier))) {
        PRTE_ERROR_LOG(rc);
        return rc;
    }
    ...
    if (prte_ras_base.launch_orted_on_hn) {
        if (NULL != hnp_node) {
            ...
            if (prte_hnp_is_allocated && !(PRTE_GET_MAPPING_DIRECTIVE(prte_rmaps_base.mapping) &
