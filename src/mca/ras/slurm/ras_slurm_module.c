...
static int init(void)
{
    ...
    if (mca_ras_slurm_component.dyn_alloc_enabled) {
        ...
        /* setup the socket */
        if (PRTE_SUCCESS != read_ip_port(mca_ras_slurm_component.config_file, &slurm_host, &port)
        ...
        if (!pmix_net_isaddr(slurm_host)) {
            ...
            if (NULL == (h = gethostbyname(slurm_host))) {
                ...
                prte_show_help("help-ras-slurm.txt", "host-not-resolved", true, slurm_host);
                free(slurm_host);
                return PRTE_ERR_SILENT;
            }
            free(slurm_host);
        ...
        if (connect(socket_fd, (struct sockaddr *) &address, sizeof(address)) < 0) {
            prte_show_help("help-ras-slurm.txt", "connection-failed", true, slurm_host, (int) port);
            free(slurm_host);
            return PRTE_ERR_SILENT;
        }
        free(slurm_host);
...
static int dyn_allocate(prte_job_t *jdata)
{
    ...
    /* pass the timeout */
    pmix_asprintf(&tmp, "timeout=%d", mca_ras_slurm_component.timeout);
    pmix_argv_append_nosize(&cmd, tmp);
    free(tmp);
