...
int prte_rmaps_base_select(void)
{
    ...
    PRTE_LIST_FOREACH(cli, &prte_rmaps_base_framework.framework_components, prte_mca_base_component_list_item_t) {
        ...
        PRTE_LIST_FOREACH(mod, &prte_rmaps_base.selected_modules, prte_rmaps_base_selected_module_t) {
            if (priority > mod->pri) {
                prte_list_insert_pos(&prte_rmaps_base.selected_modules,
    ...
    if (4 < prte_output_get_verbosity(prte_rmaps_base_framework.framework_output)) {
        ...
        PRTE_LIST_FOREACH(mod, &prte_rmaps_base.selected_modules, prte_rmaps_base_selected_module_t) {
            prte_output(0, "\tMapper: %s Priority: %d", mod->component->mca_component_name, mod->pri);
        }
    }
