...
#if !defined(PRRTE_THREADS_WAIT_SYNC_H)
#define PRRTE_THREADS_WAIT_SYNC_H
...
typedef struct prrte_wait_sync_t {
    prrte_atomic_int32_t count;
    int32_t status;
    pthread_cond_t condition;
    pthread_mutex_t lock;
    struct prrte_wait_sync_t *next;
    struct prrte_wait_sync_t *prev;
...
#define PRRTE_SYNC_WAIT(sync)    prrte_sync_wait_mt (sync)
 ...
 * the critical path. */
#define PRRTE_WAIT_SYNC_RELEASE(sync)                  \
...
#define PRRTE_WAIT_SYNC_RELEASE_NOWAIT(sync)           \
        pthread_cond_destroy(&(sync)->condition);     \
        pthread_mutex_destroy(&(sync)->lock);


#define PRRTE_WAIT_SYNC_SIGNAL(sync)                   \
        pthread_mutex_lock(&(sync->lock));            \
        pthread_cond_signal(&sync->condition);        \
        pthread_mutex_unlock(&(sync->lock));          \
        sync->signaling = false;

#define PRRTE_WAIT_SYNC_SIGNALLED(sync){               \
        (sync)->signaling = false;                    \
}

PRRTE_EXPORT int prrte_sync_wait_mt(prrte_wait_sync_t *sync);
static inline int prrte_sync_wait_st (prrte_wait_sync_t *sync)
{
    while (sync->count > 0) {
    }
...
#define PRRTE_WAIT_SYNC_INIT(sync,c)                             \
    do {                                                        \
        (sync)->count = (c);                                    \
        (sync)->next = NULL;                                    \
        (sync)->prev = NULL;                                    \
 ...
 * the waiting threads.
 */
static inline void prrte_wait_sync_update(prrte_wait_sync_t *sync,
                                         int updates, int status)
{
    if( PRRTE_LIKELY(PRRTE_SUCCESS == status) ) {
        if( 0 != (PRRTE_THREAD_ADD_FETCH32(&sync->count, -updates)) ) {
            return;
        }
    } else {
        /* this is an error path so just use the atomic */
        sync->status = PRRTE_ERROR;
        prrte_atomic_wmb ();
        prrte_atomic_swap_32 (&sync->count, 0);
    }
    PRRTE_WAIT_SYNC_SIGNAL(sync);
}

END_C_DECLS

#endif /* defined(PRRTE_THREADS_WAIT_SYNC_H) */
